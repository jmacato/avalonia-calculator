...
internal static class CldrCurrencyGenerator
{
    ...
    internal const string DefaultUnitDefinitionsPath = "src/Calculator/ViewModels/DataLoaders/UnitConverterUnits.cs";
    ...
    private static void ValidateUnitPatternSources(string unitDefinitionsPath)
    {
        ...
        string[] unitNames = Regex.Matches(
                source,
                @"^\s*public\s+static\s+UnitConverterUnits\s+(?<category>Area|Data|Energy|Length|Power|Temperature|Time|Speed|Volume|Weight|Angle|Pressure)(?<unit>[A-Z][A-Za-z0-9]*)\s+\{\s+get;\s+\}\s+=\s+new\(\d+\);",
                RegexOptions.Multiline | RegexOptions.CultureInvariant)
            .Select(match => $"{match.Groups["category"].Value}_{match.Groups["unit"].Value}")
        ...
        string[] missing = unitNames.Except(mappedNames, StringComparer.Ordinal).ToArray();
        string[] unknown = mappedNames.Except(unitNames, StringComparer.Ordinal).ToArray();
    ...
    {
        return value is { Length: 3 } && value.All(character => character is >= 'A' and <= 'Z');
    }
