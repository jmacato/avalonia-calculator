            ...
            function renderDetail(data) {
              ...
              detailNode.innerHTML = `<section><h2>${esc(s.runId || s.sessionId)}</h2><code class="subtle">${esc(data.client?.userAgent || '')}</code></section>
                <div class="metrics">
                  ...
                  ${metric('Canvas backing size', `${u.canvasWidth || 0} × ${u.canvasHeight || 0}`)} ${metric('MainPage count / root / last', `${u.mainPageCount || 0} / ${u.rootMainPageId || 0} / ${u.lastNavigatedMainPageId || 0}`)}
