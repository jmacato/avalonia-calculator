          ...
          <script>
            ...
            function renderDetail(data) {
              ...
              detailNode.innerHTML = `<section><h2>${esc(s.runId || s.sessionId)}</h2><code class="subtle">${esc(data.client?.userAgent || '')}</code></section>
                ...
                <section><h2>Recent samples</h2><table><thead><tr><th>Time</th><th>Status age</th><th>fps</th><th>gap</th><th>ptr</th><th>present</th><th>Wasm</th><th>managed</th><th>dispatcher</th></tr></thead><tbody>${timeline.map(t => `<tr><td>${esc(new Date(t.receivedAt).toLocaleTimeString())}</td><td>${age(t.uiAgeMs)}</td><td>${t.frameRate.toFixed(1)}</td><td>${age(t.maxFrameGapMs)}</td><td>${t.pointerMoveSinceLast}</td><td>${t.canvasPresentSinceLast}</td><td>${bytes(t.wasmMemoryBytes)}</td><td>${bytes(t.managedHeapBytes)}</td><td>${age(t.managedDispatcherAgeMs)}</td></tr>`).join('')}</tbody></table></section>`;
            }
