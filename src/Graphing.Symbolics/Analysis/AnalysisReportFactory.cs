...
{
    public static AnalysisReport Unknown(SemanticExpression? expression, AnalysisFeatures requested, UnknownReason reason, long work)
    {
        return new AnalysisReport(expression, UnknownFor<RealSet>(requested, AnalysisFeatures.Domain, reason),
            ...
            UnknownFor<Periodicity>(requested, AnalysisFeatures.Period, reason), work);
    }

    private static ProofOutcome<T> UnknownFor<T>(AnalysisFeatures requested, AnalysisFeatures feature, UnknownReason reason)
    {
        return ProofOutcome<T>.Unknown(requested.HasFlag(feature) ? reason : UnknownReason.NotRequested);
    }
}
