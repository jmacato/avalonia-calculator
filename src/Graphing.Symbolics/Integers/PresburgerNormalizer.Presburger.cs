...
internal static class PresburgerNormalizer
{
    ...
    {
        PresburgerDivisibility divisibility when divisibility.Divisor.IsZero => new PresburgerComparison(divisibility.Expression, IntegerRelation.Equal),
        ...
        PresburgerNot not => not with
        {
            Operand = Normalize(not.Operand)
        },
        _ => formula
    };
}
