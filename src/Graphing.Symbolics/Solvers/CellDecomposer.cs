...
{
    public static CellDecompositionCertificate Decompose(PolynomialFormula formula, ResourceBudget budget)
    {
    ...
    public static bool Verify(CellDecompositionCertificate certificate, ResourceBudget budget)
    {
        ImmutableArray<UnivariatePolynomial> expectedAtoms = PolynomialFormulaConverter.Atoms(certificate.Formula);
        ...
        combined = combined.SquareFreePart(budget).PrimitivePositive(budget);
        if (!combined.Equals(certificate.CombinedPolynomial) || !SturmRootIsolator.Verify(certificate.RootIsolation, budget) || !certificate.RootIsolation.Polynomial.Equals(combined))
        {
            return false;
        }

        ImmutableArray<CellWitness> expected = BuildWitnesses(certificate.Formula, expectedAtoms, certificate.RootIsolation.Roots, budget);
        ...
        {
            if (!string.Equals(expected[index].Canonical, certificate.Cells[index].Canonical, StringComparison.Ordinal))
            {
                return false;
            }
        }
    ...
    private static ImmutableArray<CellWitness> BuildWitnesses(PolynomialFormula formula, ImmutableArray<UnivariatePolynomial> atoms, ImmutableArray<ExactReal> roots, ResourceBudget budget)
    {
        ...
        for (int gap = 0; gap <= roots.Length; gap++)
        {
            ...
            ImmutableArray<int> intervalSigns = atoms.Select(polynomial => polynomial.Evaluate(sample, budget).Sign).ToImmutableArray();
            bool intervalIncluded = Evaluate(formula, atoms, intervalSigns);
            witnesses.Add(new CellWitness(CellKind.OpenInterval, gap, sample, intervalSigns, intervalIncluded));
            budget.AddCells(1);
            if (gap == roots.Length)
            {
                continue;
            ...
            ImmutableArray<int> pointSigns = atoms.Select(polynomial => SignAt(polynomial, roots[gap], budget)).ToImmutableArray();
            bool pointIncluded = Evaluate(formula, atoms, pointSigns);
            witnesses.Add(new CellWitness(CellKind.Point, gap, null, pointSigns, pointIncluded));
            budget.AddCells(1);
        }
    ...
    private static bool Evaluate(PolynomialFormula formula, ImmutableArray<UnivariatePolynomial> atoms, ImmutableArray<int> signs)
    {
    ...
    private static RealSet BuildSet(ImmutableArray<ExactReal> roots, ImmutableArray<CellWitness> witnesses)
    {
