...
internal static class RationalCertificateReplay
{
    ...
    internal static ImmutableArray<MonotoneRegion> ComputeMonotonicity(RationalAnalysisContext context, ResourceBudget budget, out ImmutableArray<RootIsolationCertificate> roots)
    {
        ...
        {
            roots = [context.DomainCells.RootIsolation];
            return OneDimensionalComponents(context.Domain).Select(static component => new MonotoneRegion(component, Monotonicity.Constant)).ToImmutableArray();
        }
        ...
        while (gap <= rootCount)
        {
            ...
            result.Add(new MonotoneRegion(region, sign > 0 ? Monotonicity.Increasing : Monotonicity.Decreasing));
            gap = endGap + 1;
        }
