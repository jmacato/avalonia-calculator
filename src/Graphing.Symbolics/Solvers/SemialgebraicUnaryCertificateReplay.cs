...
/// <summary>
/// Checker-owned replay of semialgebraic unary sign charts, range fibers, and
/// feature claims. It does not call the analyzer's producing proof kernel.
/// </summary>
internal static class SemialgebraicUnaryCertificateReplay
{
    ...
    private static void ComputeExtrema(SemialgebraicUnaryContext context, UnarySignChartCertificate chart, ResourceBudget budget, out ImmutableArray<FeaturePoint> minima, out ImmutableArray<FeaturePoint> maxima)
    {
        ...
        SemialgebraicUnaryCertificateReplayDifferentialData differential = Differential(context, budget);
        for (int point = 0; point < chart.RootIsolation.Roots.Length; point++)
        {
            ...
            SemialgebraicUnaryCertificateReplayExtremumKind kind = ClassifyExtremum(leftDomain, rightDomain, left, right);
            if (kind == SemialgebraicUnaryCertificateReplayExtremumKind.None)
            {
                continue;
            }
            ...
            if (kind == SemialgebraicUnaryCertificateReplayExtremumKind.Minimum)
            {
    ...
    private static SemialgebraicUnaryCertificateReplayDifferentialData Differential(SemialgebraicUnaryContext context, ResourceBudget budget)
    {
        ...
        return new SemialgebraicUnaryCertificateReplayDifferentialData(first, curvature);
    }

    private static int FirstSignInGap(SemialgebraicUnaryContext context, UnarySignChartCertificate chart, SemialgebraicUnaryCertificateReplayDifferentialData differential, int gap)
    {
    ...
    private static int CurvatureSignInGap(SemialgebraicUnaryContext context, UnarySignChartCertificate chart, SemialgebraicUnaryCertificateReplayDifferentialData differential, int gap)
    {
    ...
    private static SemialgebraicUnaryCertificateReplayExtremumKind ClassifyExtremum(bool leftDomain, bool rightDomain, int leftSign, int rightSign)
    {
        if (leftDomain && rightDomain)
        {
            ...
            {
                return SemialgebraicUnaryCertificateReplayExtremumKind.Minimum;
            }

            if (leftSign > 0 && rightSign < 0)
            {
                return SemialgebraicUnaryCertificateReplayExtremumKind.Maximum;
            }

            return SemialgebraicUnaryCertificateReplayExtremumKind.None;
        }
        ...
        if (rightDomain)
        {
            ...
            {
                > 0 => SemialgebraicUnaryCertificateReplayExtremumKind.Minimum,
                < 0 => SemialgebraicUnaryCertificateReplayExtremumKind.Maximum,
                _ => SemialgebraicUnaryCertificateReplayExtremumKind.None
            };
        }
