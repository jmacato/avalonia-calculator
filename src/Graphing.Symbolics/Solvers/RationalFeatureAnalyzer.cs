...
internal sealed record RationalAnalysisContext(
    SemanticExpression Expression,
    string Variable,
    RationalExtraction Extraction,
        ...
        out RationalAnalysisContext context)
    {
        if (!RationalFunctionExtractor.TryExtract(
            ...
            !PolynomialFormulaConverter.TryConvert(
                ...
                variable,
                budget,
                out PolynomialFormula domainFormula))
        {
            context = null!;
            return false;
        }
        ...
        context = new RationalAnalysisContext(
            expression,
            variable,
            extraction,
            domainFormula,
...
internal sealed record SignChart(
    ImmutableArray<UnivariatePolynomial> Polynomials,
    RootIsolationCertificate Roots,
    ImmutableArray<BigRational> GapSamples,
    ImmutableArray<ImmutableArray<int>> GapSigns,
    ImmutableArray<ImmutableArray<int>> PointSigns,
    ImmutableArray<bool> GapDomain,
    ImmutableArray<bool> PointDomain)
{
    public int SignInGap(UnivariatePolynomial polynomial, int gap) =>
        GapSigns[gap][IndexOf(polynomial)];

    public int SignAtPoint(UnivariatePolynomial polynomial, int point) =>
        PointSigns[point][IndexOf(polynomial)];
...
{
    public static ProofOutcome<RealSet> Zeros(
    ...
    public static ProofOutcome<OptionalValue<ExactReal>> YIntercept(
        RationalAnalysisContext context,
        ResourceBudget budget)
    {
        ...
        return Proved(
            ...
            value,
            [],
            "rational-origin-substitution");
    }

    public static ProofOutcome<FunctionParity> Parity(
        RationalAnalysisContext context,
        ResourceBudget budget)
    {
    ...
    public static ProofOutcome<RealSet> Range(
        RationalAnalysisContext context,
        ResourceBudget budget)
    {
    ...
    public static ProofOutcome<ImmutableArray<FeaturePoint>> Minima(
        RationalAnalysisContext context,
        ResourceBudget budget)
    {
        ComputeExtrema(
            context,
            budget,
            out ImmutableArray<FeaturePoint> minima,
            out _,
    ...
    public static ProofOutcome<ImmutableArray<FeaturePoint>> Maxima(
        RationalAnalysisContext context,
        ResourceBudget budget)
    {
        ComputeExtrema(
            ...
            budget,
            out _,
            out ImmutableArray<FeaturePoint> maxima,
            out ImmutableArray<RootIsolationCertificate> roots);
        return Proved(context, AnalysisFeatures.Maxima, maxima, roots, "derivative-sign-cell-classification");
    }

    public static ProofOutcome<ImmutableArray<FeaturePoint>> Inflections(
        RationalAnalysisContext context,
        ResourceBudget budget)
    {
        ImmutableArray<FeaturePoint> value = ComputeInflections(
            context,
            budget,
            out ImmutableArray<RootIsolationCertificate> roots);
        return Proved(
            ...
            value,
            roots,
            "two-sided-second-derivative-sign-change");
    }

    public static ProofOutcome<ImmutableArray<MonotoneRegion>> Monotonicity(
        RationalAnalysisContext context,
        ResourceBudget budget)
    {
        ImmutableArray<MonotoneRegion> value = ComputeMonotonicity(
            context,
            budget,
            out ImmutableArray<RootIsolationCertificate> roots);
        return Proved(
            ...
            value,
            roots,
            "maximal-domain-derivative-sign-cells");
    }

    public static ProofOutcome<ImmutableArray<Asymptote>> VerticalAsymptotes(
        RationalAnalysisContext context,
        ResourceBudget budget)
    {
        ImmutableArray<Asymptote> value = ComputeVerticalAsymptotes(
            context,
            budget,
            out ImmutableArray<RootIsolationCertificate> roots);
        return Proved(
            ...
            value,
            roots,
            "reduced-denominator-poles");
    }

    public static ProofOutcome<ImmutableArray<Asymptote>> HorizontalAsymptotes(
        RationalAnalysisContext context,
        ResourceBudget budget)
    {
        ...
        return Proved(
            ...
            value,
            [],
            "rational-degree-limit");
    }

    public static ProofOutcome<ImmutableArray<Asymptote>> ObliqueAsymptotes(
        RationalAnalysisContext context,
        ResourceBudget budget)
    {
        ...
        return Proved(
            ...
            value,
            [],
            "rational-polynomial-division-limit");
    }

    public static ProofOutcome<Periodicity> Period(
        RationalAnalysisContext context,
        ResourceBudget budget)
    {
        ...
        return Proved(
            ...
            value,
            [],
            "nonconstant-rational-functions-have-no-real-period");
    }

    internal static RealSet ComputeZeros(
        RationalAnalysisContext context,
        ResourceBudget budget,
        out ImmutableArray<RootIsolationCertificate> roots)
    {
        ...
        PolynomialAtom zero = new(
            context.Function.Numerator.PrimitivePositive(budget),
            Comparison.Equal);
    ...
    internal static OptionalValue<ExactReal> ComputeYIntercept(
        RationalAnalysisContext context,
        ResourceBudget budget)
    {
        ...
        return OptionalValue<ExactReal>.Some(
            new RationalReal(context.Function.Evaluate(BigRational.Zero, budget)));
    }

    internal static FunctionParity ComputeParity(
        RationalAnalysisContext context,
        ResourceBudget budget,
        out ImmutableArray<RootIsolationCertificate> roots)
    {
        PolynomialFormula reflectedDomain = SubstituteNegative(context.DomainFormula, budget);
        PolynomialFormula asymmetric = new PolynomialJunction(false,
        [
            new PolynomialJunction(true, [context.DomainFormula, new PolynomialNot(reflectedDomain)]),
            new PolynomialJunction(true, [new PolynomialNot(context.DomainFormula), reflectedDomain])
        ]);
    ...
    internal static bool TryComputeRange(
        RationalAnalysisContext context,
        ResourceBudget budget,
        out RealSet range,
        out ImmutableArray<RootIsolationCertificate> roots)
    {
        ...
        if (function.Denominator.Degree == 0 && context.Domain is AllRealSet)
        {
            ...
            if (polynomial.Degree == 2)
            {
                ...
                range = new IntervalSet(
                    minimum ? RealBound.Finite(y) : RealBound.NegativeInfinity,
                    minimum,
                    minimum ? RealBound.PositiveInfinity : RealBound.Finite(y),
                    !minimum);
                return true;
            }
        ...
        if (function.Numerator.Degree == 1 &&
            ...
            DomainIsExactlyReducedDenominator(context, budget))
        {
    ...
    internal static void ComputeExtrema(
        RationalAnalysisContext context,
        ResourceBudget budget,
        out ImmutableArray<FeaturePoint> minima,
        out ImmutableArray<FeaturePoint> maxima,
        out ImmutableArray<RootIsolationCertificate> roots)
    {
        ...
        SignChart chart = BuildSignChart(context, [derivative.Numerator], budget);
        ...
        {
            if (!chart.PointDomain[point] || chart.SignAtPoint(derivative.Numerator, point) != 0)
            {
                continue;
            }
            ...
            int leftSign = leftDomain ? chart.SignInGap(derivative.Numerator, point) : 0;
            int rightSign = rightDomain ? chart.SignInGap(derivative.Numerator, point + 1) : 0;
            ExactReal x = chart.Roots.Roots[point];
            var featurePoint = new FeaturePoint(
                new SingletonReal(x),
                EvaluateExact(context.Function, x, budget));

            ExtremumKind kind = ClassifyExtremum(leftDomain, rightDomain, leftSign, rightSign);
            if (kind == ExtremumKind.Minimum)
            {
                minimumBuilder.Add(featurePoint);
            }

            if (kind == ExtremumKind.Maximum)
            {
    ...
    internal static ImmutableArray<FeaturePoint> ComputeInflections(
        RationalAnalysisContext context,
        ResourceBudget budget,
        out ImmutableArray<RootIsolationCertificate> roots)
    {
        ...
        SignChart chart = BuildSignChart(context, [second.Numerator], budget);
        var result = ImmutableArray.CreateBuilder<FeaturePoint>();
        for (int point = 0; point < chart.Roots.Roots.Length; point++)
        {
            if (!chart.PointDomain[point] ||
                !chart.GapDomain[point] ||
                !chart.GapDomain[point + 1] ||
                chart.SignAtPoint(second.Numerator, point) != 0)
            {
                continue;
            }

            int left = chart.SignInGap(second.Numerator, point);
            int right = chart.SignInGap(second.Numerator, point + 1);
    ...
    internal static ImmutableArray<MonotoneRegion> ComputeMonotonicity(
        RationalAnalysisContext context,
        ResourceBudget budget,
        out ImmutableArray<RootIsolationCertificate> roots)
    {
        ...
        {
            roots = [context.DomainCells.RootIsolation];
            return OneDimensionalComponents(context.Domain)
                .Select(static component => new MonotoneRegion(
                    component,
        ...
        for (int gap = 0; gap <= chart.Roots.Roots.Length; gap++)
        {
            ...
            int sign = chart.SignInGap(derivative.Numerator, gap);
            if (sign == 0)
            {
                continue;
            }

            RealBound lower = gap == 0
                ? RealBound.NegativeInfinity
                : RealBound.Finite(chart.Roots.Roots[gap - 1]);
            RealBound upper = gap == chart.Roots.Roots.Length
                ? RealBound.PositiveInfinity
                : RealBound.Finite(chart.Roots.Roots[gap]);
            result.Add(new MonotoneRegion(
                new IntervalSet(lower, false, upper, false),
                sign > 0
    ...
    internal static ImmutableArray<Asymptote> ComputeVerticalAsymptotes(
        RationalAnalysisContext context,
        ResourceBudget budget,
        out ImmutableArray<RootIsolationCertificate> roots)
    {
        ...
        RootIsolationCertificate isolation = SturmRootIsolator.Isolate(
            ...
            budget);
        roots = [isolation];
        return isolation.Roots
            .Select(static root => new Asymptote(
                AsymptoteOrientation.Vertical,
                new SingletonReal(root),
                null,
                null))
            .ToImmutableArray();
    }

    internal static ImmutableArray<Asymptote> ComputeHorizontalAsymptotes(
        RationalAnalysisContext context)
    {
        RationalFunction function = context.Function;
        if (!HasUnboundedComponent(context.Domain) ||
            function.Numerator.Degree > function.Denominator.Degree)
        {
            return [];
        }

        bool constant = function.Numerator.Degree <= 0 &&
        ...
        BigRational value = function.Numerator.Degree == function.Denominator.Degree
        ...
        [
            new Asymptote(
                ...
                new SingletonReal(new RationalReal(value)),
                null,
                new RationalReal(value))
        ];
    }

    internal static ImmutableArray<Asymptote> ComputeObliqueAsymptotes(
        RationalAnalysisContext context,
        ResourceBudget budget)
    {
        RationalFunction function = context.Function;
        if (!HasUnboundedComponent(context.Domain) ||
            function.Numerator.Degree - function.Denominator.Degree != 1)
        {
            return [];
        }
    ...
    internal static Periodicity ComputePeriod(RationalAnalysisContext context) =>
        context.Function.Numerator.Degree <= 0 &&
        context.Function.Denominator.Degree <= 0 &&
    ...
    private static ProofOutcome<T> Proved<T>(
        RationalAnalysisContext context,
        AnalysisFeatures feature,
        T value,
        ImmutableArray<RootIsolationCertificate> roots,
        string rule)
    {
        var certificate = new RationalFunctionProofCertificate(
            feature,
    ...
    private static SignChart BuildSignChart(
        RationalAnalysisContext context,
        ImmutableArray<UnivariatePolynomial> extras,
        ResourceBudget budget)
    {
        ImmutableArray<UnivariatePolynomial> domainAtoms =
            PolynomialFormulaConverter.Atoms(context.DomainFormula);
        ImmutableArray<UnivariatePolynomial> polynomials = domainAtoms
            .Concat(extras.Where(static polynomial => !polynomial.IsZero))
        ...
        for (int gap = 0; gap <= roots.Roots.Length; gap++)
        {
            ...
            ImmutableArray<int> signs = polynomials
            ...
            ImmutableArray<int> atPoint = polynomials
                .Select(polynomial => SignAt(polynomial, root, budget))
                .ToImmutableArray();
            pointSigns.Add(atPoint);
            pointDomain.Add(EvaluateFormula(context.DomainFormula, polynomials, atPoint));
        }

        return new SignChart(
            polynomials,
            roots,
            gapSamples.MoveToImmutable(),
            gapSigns.MoveToImmutable(),
            pointSigns.MoveToImmutable(),
            gapDomain.MoveToImmutable(),
            pointDomain.MoveToImmutable());
    }

    private static bool EvaluateFormulaAt(
    ...
    private static bool EvaluateFormula(
        PolynomialFormula formula,
        ImmutableArray<UnivariatePolynomial> polynomials,
    ...
    private static PolynomialFormula SubstituteNegative(
        PolynomialFormula formula,
        ResourceBudget budget) => formula switch
    {
        ...
        PolynomialJunction junction => new PolynomialJunction(
            junction.IsConjunction,
    ...
    private static PolynomialAtom NormalizeReflectedAtom(
        PolynomialAtom atom,
        ResourceBudget budget)
    {
        UnivariatePolynomial reflected = atom.Polynomial.SubstituteNegativeVariable(budget);
        Comparison comparison = reflected.LeadingCoefficient.Sign < 0
            ? Reverse(atom.Comparison)
            : atom.Comparison;
    ...
    private static bool DomainIsExactlyReducedDenominator(
        RationalAnalysisContext context,
        ResourceBudget budget)
    {
    ...
    private static ExactReal EvaluateExact(
        RationalFunction function,
        ExactReal value,
        ResourceBudget budget) => value switch
    {
    ...
    private static ExtremumKind ClassifyExtremum(
        bool leftDomain,
        bool rightDomain,
        int leftSign,
        int rightSign)
    {
        if (leftDomain && rightDomain)
        {
            ...
            {
                return ExtremumKind.Minimum;
            }

            if (leftSign > 0 && rightSign < 0)
            {
                return ExtremumKind.Maximum;
            }

            return ExtremumKind.None;
        }
