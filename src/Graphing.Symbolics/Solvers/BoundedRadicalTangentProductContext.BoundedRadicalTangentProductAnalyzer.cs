...
internal sealed record BoundedRadicalTangentProductContext(SemanticExpression Expression, SemanticExpression Variable, SemanticExpression LowerRadical, SemanticExpression UpperRadical, SemanticExpression Tangent, BigRational UpperEndpoint, IntervalSet Domain, string SourceShapeCanonical)
{
    ...
    private static bool IsBareVariable(SemanticExpression expression, string variable) => expression.Value is { Kind: ValueKind.Variable, Name: var name, Operands.IsEmpty: true } && name.Equals(variable, StringComparison.OrdinalIgnoreCase) && expression.SourceOperands.IsEmpty && expression.RewriteHistory.IsEmpty && CanonicalEquals(expression.DefinedWhen, Formula.True) && CanonicalEquals(expression.ContinuousWhen, Formula.True) && CanonicalEquals(expression.DifferentiableWhen, Formula.True);
    ...
    private static string BinaryKindName(ValueKind kind) => kind switch
    {
        ValueKind.Add => "add",
    ...
    private static bool TryMatchExactConstantRewrite(SemanticExpression expression, string rule, string before) => expression.RewriteHistory is [var rewrite] && string.Equals(rewrite.Rule, rule, StringComparison.Ordinal) && string.Equals(rewrite.Before, before, StringComparison.Ordinal) && string.Equals(rewrite.After, expression.Value.Canonical, StringComparison.Ordinal) && rewrite.Guard == Formula.True;
    private static bool HasExactEverywhereRegularity(SemanticExpression expression) => CanonicalEquals(expression.DefinedWhen, Formula.True) && CanonicalEquals(expression.ContinuousWhen, Formula.True) && CanonicalEquals(expression.DifferentiableWhen, Formula.True);
    private static string SourceTermCanonical(ValueKind kind, params ValueTerm[] operands) => $"{(int)kind}:({string.Join(',', operands.Select(static value => value.Canonical))})";
    private static bool HasInheritedRegularity(SemanticExpression expression, SemanticExpression left, SemanticExpression right) => CanonicalEquals(expression.DefinedWhen, Formula.And(left.DefinedWhen, right.DefinedWhen)) && CanonicalEquals(expression.ContinuousWhen, Formula.And(left.ContinuousWhen, right.ContinuousWhen)) && CanonicalEquals(expression.DifferentiableWhen, Formula.And(left.DifferentiableWhen, right.DifferentiableWhen));
    private static bool SameVariable(SemanticExpression? established, SemanticExpression candidate) => established is null || established.Value.Id == candidate.Value.Id;
    private static bool CanonicalEquals(Formula actual, Formula expected) => string.Equals(actual.Canonical, expected.Canonical, StringComparison.Ordinal);
    private static ValueTerm FunctionTerm(string name, ValueTerm argument) => new(-1, ValueKind.Function, default, name, [argument], $"{(int)ValueKind.Function}:{name}({argument.Canonical})");
}
