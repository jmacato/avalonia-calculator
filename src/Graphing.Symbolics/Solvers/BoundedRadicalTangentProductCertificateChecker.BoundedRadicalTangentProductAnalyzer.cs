...
internal static class BoundedRadicalTangentProductCertificateChecker
{
    ...
    private static bool MatchConstantRewrite(SemanticExpression expression, string rule, string before) => expression.RewriteHistory is [var rewrite] && string.Equals(rewrite.Rule, rule, StringComparison.Ordinal) && string.Equals(rewrite.Before, before, StringComparison.Ordinal) && string.Equals(rewrite.After, expression.Value.Canonical, StringComparison.Ordinal) && rewrite.Guard == Formula.True;
    private static bool IsBareVariable(SemanticExpression expression, string variable) => expression.Value is { Kind: ValueKind.Variable, Name: var name, Operands.IsEmpty: true } && name.Equals(variable, StringComparison.OrdinalIgnoreCase) && expression.SourceOperands.IsEmpty && expression.RewriteHistory.IsEmpty && HasExactEverywhereRegularity(expression);
