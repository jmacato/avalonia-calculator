...
internal static class RealSetIntersectionSolver
{
    ...
    private static bool HasRationalOrInfiniteBounds(IntervalSet interval)
    {
        return (interval.Lower.Kind != BoundKind.Finite || interval.Lower.Value is RationalReal) &&
               (interval.Upper.Kind != BoundKind.Finite || interval.Upper.Value is RationalReal);
    }
    ...
    {
        return set.Period is AffinePiReal { PiCoefficient: var coefficient, Constant.IsZero: true } &&
               coefficient == BigRational.One && set.Intervals.Length == 1 &&
               set.Intervals[0].LowerOffset is AffinePiReal { PiCoefficient: var lower, Constant.IsZero: true } &&
               lower == new BigRational(-1, 2) &&
               set.Intervals[0].UpperOffset is AffinePiReal { PiCoefficient: var upper, Constant.IsZero: true } &&
    ...
    private static bool TryMaximumLower(IntervalSet left, IntervalSet right, out RealBound bound, out bool included)
    {
        return TrySelectBound(left.Lower, left.IncludesLower, right.Lower, right.IncludesLower, maximum: true,
            out bound, out included);
    }

    private static bool TryMinimumUpper(IntervalSet left, IntervalSet right, out RealBound bound, out bool included)
    {
        return TrySelectBound(left.Upper, left.IncludesUpper, right.Upper, right.IncludesUpper, maximum: false,
            out bound, out included);
    }
