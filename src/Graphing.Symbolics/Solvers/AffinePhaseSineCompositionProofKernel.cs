...
internal static class AffinePhaseSineCompositionProofKernel
{
    ...
    private static PeriodicIntervalSet Domain(AffinePhaseSineCompositionPattern pattern, AngleUnit angleUnit)
    {
    ...
    private static FunctionParity Parity(AffinePhaseSineCompositionPattern pattern, AngleUnit angleUnit)
    {
    ...
    private static PeriodicPointSet Zeros(AffinePhaseSineCompositionPattern pattern, AngleUnit angleUnit)
    {
    ...
    {
        BigRational start = pattern.InnerSign > 0 ? BigRational.Zero : BigRational.One;
        return [new MonotoneRegion(PeriodicIntervals(pattern, angleUnit, start, false, start + new BigRational(1, 2), false, new BigRational(2)), Monotonicity.Increasing), new MonotoneRegion(PeriodicIntervals(pattern, angleUnit, start + new BigRational(1, 2), false, start + BigRational.One, false, new BigRational(2)), Monotonicity.Decreasing)];
    }

    private static Periodicity Period(AffinePhaseSineCompositionPattern pattern, AngleUnit angleUnit)
    {
        return new Periodicity(PeriodicityKind.PeriodicWithFundamentalPeriod, Step(pattern, angleUnit, new BigRational(2)));
    }

    private static ConstantYFeaturePoint PeriodicFeature(AffinePhaseSineCompositionPattern pattern, AngleUnit angleUnit, BigRational angle, ExactReal period, ExactReal y)
    {
        return new ConstantYFeaturePoint(
            new PeriodicReal(SolveAngle(pattern, Angle(angleUnit, angle)), period, Parameter,
                IntegerConstraint.All(Parameter)), y);
    }

    private static PeriodicIntervalSet PeriodicIntervals(AffinePhaseSineCompositionPattern pattern, AngleUnit angleUnit, BigRational lower, bool includesLower, BigRational upper, bool includesUpper, BigRational period)
    {
        return new PeriodicIntervalSet(Step(pattern, angleUnit, period), Parameter, IntegerConstraint.All(Parameter),
        [
            new PeriodicInterval(SolveAngle(pattern, Angle(angleUnit, lower)), includesLower,
                SolveAngle(pattern, Angle(angleUnit, upper)), includesUpper)
        ]);
    }

    private static ExactReal SolveAngle(AffinePhaseSineCompositionPattern pattern, ExactReal angle)
    {
        return ExactRealArithmetic.Scale(ExactRealArithmetic.Subtract(angle, pattern.Phase),
            pattern.Frequency.Reciprocal());
    }

    private static ExactReal Step(AffinePhaseSineCompositionPattern pattern, AngleUnit angleUnit, BigRational fraction)
    {
