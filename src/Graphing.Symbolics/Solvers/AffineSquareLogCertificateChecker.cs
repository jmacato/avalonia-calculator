...
internal static class AffineSquareLogCertificateChecker
{
    ...
    private static bool HasValidEnvelope(AnalysisRequest request, SemanticExpression expression, AffineSquareLogProofCertificate certificate, string claim)
    {
        return certificate.Rule == Rule && IsSingleFeature(certificate.Feature) &&
               certificate.Feature == certificate.ProvenFeature && request.Features.HasFlag(certificate.Feature) &&
               ...
               string.Equals(certificate.ClaimCanonical, claim, StringComparison.Ordinal) && string.Equals(
    ...
    private static PointSet Zeros(AffineSquareLogCertificateCheckerAffineSquareLogReplayContext context, ResourceBudget budget)
    {
