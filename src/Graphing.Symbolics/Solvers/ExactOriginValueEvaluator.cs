...
/// </summary>
internal sealed class ExactOriginValueEvaluator(string variable, AngleUnit angleUnit, ResourceBudget budget)
{
    ...
    public bool TryEvaluate(Formula formula, out bool value)
    {
        ...
        switch (formula)
        {
            ...
            case ComparisonFormula comparison when TryEvaluate(comparison.Left, out ExactReal left) && TryEvaluate(comparison.Right, out ExactReal right):
                return ExactRealSubstitutionArithmetic.TryCompare(left, comparison.Comparison, right, budget, out value);
    ...
    private bool TryEvaluateCore(ValueTerm term, out ExactReal value)
    {
        switch (term.Kind)
        {
            ...
            case ValueKind.Negate when term.Operands is [var operand] && TryEvaluate(operand, out ExactReal negated):
                value = ExactRealSubstitutionArithmetic.Negate(negated, budget);
                return true;
            case ValueKind.Add:
    ...
    private bool TryEvaluateBinary(ValueTerm term, out ExactReal value)
    {
        ...
        {
            ValueKind.Add => ExactRealSubstitutionArithmetic.Add(left, right, budget),
            ValueKind.Subtract => ExactRealSubstitutionArithmetic.Subtract(left, right, budget),
            ValueKind.Multiply => ExactRealSubstitutionArithmetic.Multiply(left, right, budget),
            ValueKind.Divide => ExactRealSubstitutionArithmetic.Divide(left, right, budget),
    ...
    private bool TryEvaluatePower(ValueTerm term, out ExactReal value)
    {
        ...
        if (exponentValue is RationalReal exponent)
        {
            ...
            value = ExactRealSubstitutionArithmetic.Power(basis, exponent.Value, budget);
            return true;
        }
    ...
    private bool TryEvaluateFunction(ValueTerm term, out ExactReal value)
    {
        ...
        {
            case "abs":
                value = ExactRealSubstitutionArithmetic.Absolute(argument, budget);
                return true;
            case "sqrt":
                value = ExactRealSubstitutionArithmetic.SquareRoot(argument, budget);
                return true;
            case "sin":
            case "cos":
            case "tan":
    ...
    private bool TryEvaluateExtremum(ValueTerm term, out ExactReal value)
    {
        ...
        for (int index = 1; index < arguments.Count; index++)
        {
            ...
            if (!ExactRealSubstitutionArithmetic.TryCompare(candidate, comparison, selected, budget, out bool precedes))
            {
    ...
    private bool TryEvaluateRemainingFunction(string function, ExactReal argument, out ExactReal value)
    {
        ...
        if (function == "sign")
        {
            value = ExactRealSubstitutionArithmetic.TrySign(argument, budget, out int sign) ? new RationalReal(new BigRational(sign)) : new FunctionReal(function, [argument]);
            return true;
        }
    ...
    private bool TryEvaluateRoot(ValueTerm term, out ExactReal value)
    {
        ...
        if (degreeValue is RationalReal degree)
        {
            ...
            value = ExactRealSubstitutionArithmetic.Root(radicand, degree.Value, budget);
            return true;
        }
    ...
    {
        ExactReal radians = ExactRealSubstitutionArithmetic.ValidateCoefficients(ExactAngleArithmetic.ToRadians(argument, angleUnit), budget);
        if (IsZero(radians))
        {
    ...
    private ExactReal EvaluateInverseTrigonometric(string function, ExactReal argument)
    {
        return ExactRealSubstitutionArithmetic.ValidateCoefficients(
            argument is RationalReal rational
                ? ExactInverseTrigonometry.PrincipalAngle(function, ExactScalar.FromRational(rational.Value),
                    angleUnit)
                : ExactAngleArithmetic.FromRadians(new FunctionReal(function, [argument]), angleUnit), budget);
    }
    ...
    private bool TryFactorial(ExactReal argument, out BigRational factorial)
    {
        ...
        for (int factor = 2; factor <= limit; factor++)
        {
            ...
            budget.CheckCoefficient(factorial);
        }
    ...
    private static bool IsOne(ExactReal value)
    {
