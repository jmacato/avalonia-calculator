...
internal static class FourierReducer
{
    ...
    private static FourierPolynomial Map(FourierPolynomial source, Func<GaussianRational, GaussianRational> transform) => new(source.Coefficients.ToImmutableSortedDictionary(static item => item.Key, item => transform(item.Value)));
    private static FourierPolynomial From(params (int Frequency, GaussianRational Coefficient)[] values) => new(values.Where(static item => !item.Coefficient.IsZero).ToImmutableSortedDictionary(static item => item.Frequency, static item => item.Coefficient));
