...
/// <summary>
/// Checker-owned replay of fixed rational-power sign charts, range fibers, and
...
internal static class FixedRationalPowerCertificateReplay
{
    ...
    private static void ComputeExtrema(FixedRationalPowerContext context, CellDecompositionCertificate chart, ResourceBudget budget, out ImmutableArray<FeaturePoint> minima, out ImmutableArray<FeaturePoint> maxima)
    {
        ...
        FixedRationalPowerCertificateReplayDifferentialData differential = Differential(context, budget);
        for (int point = 0; point < chart.RootIsolation.Roots.Length; point++)
        {
            ...
            FixedRationalPowerCertificateReplayExtremumKind kind = ClassifyExtremum(leftDomain, rightDomain, left, right);
            if (kind == FixedRationalPowerCertificateReplayExtremumKind.None)
            {
                continue;
            }
            ...
            if (kind == FixedRationalPowerCertificateReplayExtremumKind.Minimum)
            {
    ...
    private static FixedRationalPowerCertificateReplayDifferentialData Differential(FixedRationalPowerContext context, ResourceBudget budget)
    {
    ...
    private static FixedRationalPowerCertificateReplayExtremumKind ClassifyExtremum(bool leftDomain, bool rightDomain, int leftSign, int rightSign)
    {
        if (leftDomain && rightDomain)
        {
            if (leftSign < 0 && rightSign > 0)
            {
                return FixedRationalPowerCertificateReplayExtremumKind.Minimum;
            }

            if (leftSign > 0 && rightSign < 0)
            {
                return FixedRationalPowerCertificateReplayExtremumKind.Maximum;
            }

            return FixedRationalPowerCertificateReplayExtremumKind.None;
        }
        ...
        if (rightDomain)
        {
            ...
            {
                > 0 => FixedRationalPowerCertificateReplayExtremumKind.Minimum,
                < 0 => FixedRationalPowerCertificateReplayExtremumKind.Maximum,
                _ => FixedRationalPowerCertificateReplayExtremumKind.None
            };
        }
