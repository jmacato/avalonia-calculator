...
internal static class GuardedCotangentIdentityCertificateChecker
{
    ...
    private static bool HasValidEnvelope(AnalysisRequest request, SemanticExpression expression, GuardedCotangentIdentityProofCertificate certificate, string claim)
    {
        return certificate.Feature == certificate.ProvenFeature && IsSingleFeature(certificate.Feature) &&
    ...
    private static bool HasExactTangentGuard(Formula definedWhen, AffineTrigPattern tangent, string variable, ResourceBudget budget)
    {
        return ExactFormulaVerifier.MatchesSingleGuard(definedWhen,
            operand => MatchesCosineGuard(operand, tangent, variable, budget), budget);
    }
    ...
    private static bool MatchesDenominatorNonzeroGuard(Formula formula, ValueTerm denominator)
    {
        return formula is ComparisonFormula
        {
            Comparison: Comparison.NotEqual, Left: var candidate,
            Right: { Kind: ValueKind.Constant, Constant.IsZero: true }
        } && candidate.Id == denominator.Id;
    }
