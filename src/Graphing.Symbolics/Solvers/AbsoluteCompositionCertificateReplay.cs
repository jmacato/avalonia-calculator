...
/// <summary>
/// Independent replay for absolute-composition certificates.  The checker
/// reconstructs the source pattern, proves that the retained source domain is
/// the whole line, and derives the feature claim without entering the
/// analyzer's extraction or theorem control flow.
/// </summary>
internal static class AbsoluteCompositionCertificateReplay
{
    private const string Rule = "exact-absolute-affine-composition";
    private const string Parameter = "m";

    public static bool Check(
        AnalysisRequest request,
        SemanticExpression expression,
        AbsoluteCompositionProofCertificate certificate,
        string claim,
        ResourceBudget budget)
    {
        ...
        if (certificate.Rule != Rule ||
            ...
            !TryProveAllRealDomain(expression.DefinedWhen, request.Variable, budget) ||
            !TryExtractPattern(
                expression.Value,
                request.Variable,
                budget,
                out AbsoluteCompositionPattern? pattern) ||
            pattern.Form != certificate.Form ||
    ...
    private static bool TryProveAllRealDomain(
        Formula definedWhen,
        string variable,
        ResourceBudget budget)
    {
        budget.Charge();
        if (definedWhen is BooleanFormula { Value: true })
        {
            return true;
        }
        ...
        return CellDecomposer.Verify(cells, budget) &&
               cells.Result is AllRealSet;
    }
    ...
    {
        budget.Charge();
        if (!TryExtractAbsoluteOuter(term, variable, budget, out pattern) &&
            !TryExtractAbsoluteInner(term, variable, budget, out pattern))
        {
            pattern = null!;
            return false;
        }
    ...
    private static bool TryExtractAbsoluteOuter(
        ValueTerm term,
        string variable,
        ResourceBudget budget,
        out AbsoluteCompositionPattern pattern)
    {
        ...
        if (!TryStripScale(operand, budget, out ValueTerm core, out ExactScalar scale) ||
            core is not { Kind: ValueKind.Function, Operands.Length: 1 } ||
            core.Name is not ("sin" or "cos" or "sinh" or "cosh" or "tanh") ||
            !TryExtractAffineArgument(
                core.Operands[0],
                variable,
                budget,
        ...
        pattern = new AbsoluteCompositionPattern(
            AbsoluteCompositionForm.AbsoluteOfAffineFunction,
            core.Name,
            scale.Abs(),
            slope,
            intercept);
    ...
    private static bool TryExtractAbsoluteInner(
        ValueTerm term,
        string variable,
        ResourceBudget budget,
        out AbsoluteCompositionPattern pattern)
    {
        if (term is not { Kind: ValueKind.Function, Operands.Length: 1 } ||
            term.Name is not ("sin" or "cos" or "sinh" or "cosh" or "tanh"))
        {
            pattern = null!;
            return false;
        }
        ...
        pattern = new AbsoluteCompositionPattern(
            AbsoluteCompositionForm.FunctionOfAbsoluteAffine,
            term.Name,
            ExactScalar.One,
            slope,
            intercept);
    ...
    private static bool TryStripScale(
        ValueTerm term,
        ResourceBudget budget,
        out ValueTerm core,
        ...
        do
        {
                     ...
                     ExactScalar.TryCreate(core.Operands[1], budget, out ExactScalar right))
            {
                ...
                changed = true;
                     ...
                     ExactScalar.TryCreate(core.Operands[1], budget, out ExactScalar denominator) &&
                     !denominator.IsZero)
            {
    ...
    private static bool TryExtractAffineArgument(
        ValueTerm term,
        string variable,
        ResourceBudget budget,
        out BigRational slope,
    ...
    private static bool TryReconstructClaim(
        AbsoluteCompositionPattern pattern,
        AngleUnit angleUnit,
        ...
        out object value)
    {
        ...
        {
            AbsoluteCompositionForm.AbsoluteOfAffineFunction =>
                ReconstructAbsoluteOuter(pattern, angleUnit, feature),
            AbsoluteCompositionForm.FunctionOfAbsoluteAffine =>
                ReconstructAbsoluteInner(pattern, angleUnit, feature),
            _ => null!
        };
        return value is not null;
    }

    private static object ReconstructAbsoluteOuter(
        AbsoluteCompositionPattern pattern,
        AngleUnit angleUnit,
        AnalysisFeatures feature) => feature switch
        {
            ...
            AnalysisFeatures.YIntercept => OptionalValue<ExactReal>.Some(
                AbsoluteOuterAtOrigin(pattern, angleUnit)),
    ...
    private static object ReconstructAbsoluteInner(
        AbsoluteCompositionPattern pattern,
        AngleUnit angleUnit,
        AnalysisFeatures feature)
    {
        ...
        {
            return ReconstructSineAbsoluteInner(pattern, angleUnit, feature);
        }
        ...
        return feature switch
        {
            ...
            AnalysisFeatures.YIntercept => OptionalValue<ExactReal>.Some(
                AbsoluteInnerAtOrigin(pattern, angleUnit)),
    ...
    private static object ReconstructSineAbsoluteInner(
        AbsoluteCompositionPattern pattern,
        AngleUnit angleUnit,
        AnalysisFeatures feature) => feature switch
        {
    ...
    private static Graphing.Symbolics.IntervalSet AbsoluteOuterRange(AbsoluteCompositionPattern pattern)
    {
        ...
        {
            "sin" or "cos" => new IntervalSet(
                RealBound.Finite(zero),
                true,
                RealBound.Finite(magnitude),
                true),
            "tanh" => new IntervalSet(
                RealBound.Finite(zero),
                true,
                RealBound.Finite(magnitude),
                false),
            "cosh" => new IntervalSet(
                RealBound.Finite(magnitude),
                true,
    ...
    private static Graphing.Symbolics.IntervalSet AbsoluteInnerRange(AbsoluteCompositionPattern pattern) =>
    ...
    private static Graphing.Symbolics.IntervalSet ClosedInterval(BigRational lower, BigRational upper) =>
        new IntervalSet(
            RealBound.Finite(Rational(lower)),
            true,
            RealBound.Finite(Rational(upper)),
            true);

    private static FunctionParity AbsoluteOuterParity(
        AbsoluteCompositionPattern pattern,
        AngleUnit angleUnit)
    {
        if (pattern.Function is "sin" or "cos" &&
            TryQuarterTurnMultiple(pattern.Intercept, angleUnit, out _))
        {
    ...
    private static FunctionParity AbsoluteInnerParity(
        AbsoluteCompositionPattern pattern,
        AngleUnit angleUnit)
    {
        if (pattern.Function == "cos" &&
            TryQuarterTurnMultiple(pattern.Intercept, angleUnit, out bool oddMultiple))
        {
    ...
    private static FunctionParity CenteredParity(AbsoluteCompositionPattern pattern) =>
        pattern.Intercept.IsZero ? FunctionParity.Even : FunctionParity.Neither;

    private static bool TryQuarterTurnMultiple(
        BigRational phase,
        AngleUnit angleUnit,
    ...
    private static RealSet AbsoluteOuterZeros(
        AbsoluteCompositionPattern pattern,
        AngleUnit angleUnit) => pattern.Function switch
        {
    ...
    private static RealSet AbsoluteInnerZeros(
        AbsoluteCompositionPattern pattern,
        AngleUnit angleUnit) => pattern.Function switch
        {
            "cos" => TrigPointSet(pattern, angleUnit, new BigRational(1, 2), 1),
    ...
    private static ExactReal AbsoluteOuterAtOrigin(
        AbsoluteCompositionPattern pattern,
        AngleUnit angleUnit)
    {
        ExactReal primitive = PrimitiveAtRational(pattern.Function, pattern.Intercept, angleUnit);
        ExactReal absolute = pattern.Function == "cosh"
            ? primitive
            : Absolute(primitive);
        return ExactRealArithmetic.Multiply(pattern.Scale.Value, absolute);
    }

    private static ExactReal AbsoluteInnerAtOrigin(
        AbsoluteCompositionPattern pattern,
        AngleUnit angleUnit) =>
        PrimitiveAtRational(pattern.Function, pattern.Intercept.Abs(), angleUnit);

    private static ExactReal PrimitiveAtRational(
        string function,
        ...
        AngleUnit angleUnit)
    {
        if (argument.IsZero)
        {
            return function is "cos" or "cosh"
        ...
        ExactReal exactArgument = Rational(argument);
    ...
    {
        RationalReal rational => Rational(rational.Value.Abs()),
        FunctionReal { Function: "abs", Arguments.Length: 1 } function => function,
        _ => new FunctionReal("abs", [value])
    };

    private static ImmutableArray<FeaturePoint> AbsoluteOuterExtrema(
        AbsoluteCompositionPattern pattern,
        AngleUnit angleUnit,
        bool minimum)
    {
        if (pattern.Function is "sin" or "cos")
        {
            ...
            ExactReal y = minimum ? Rational(BigRational.Zero) : pattern.Scale.Value;
            return
            [
                PeriodicFeature(
                    pattern,
        ...
        if (minimum)
        {
            ExactReal y = pattern.Function == "cosh"
                ? pattern.Scale.Value
                : Rational(BigRational.Zero);
    ...
    private static ImmutableArray<FeaturePoint> AbsoluteInnerExtrema(
        AbsoluteCompositionPattern pattern,
        AngleUnit angleUnit,
        bool minimum)
    {
        if (pattern.Function == "cos")
        {
            ...
            ExactReal y = Rational(minimum ? BigRational.MinusOne : BigRational.One);
            return
            [
                PeriodicFeature(
                    pattern,
    ...
    private static ImmutableArray<FeaturePoint> AbsoluteInnerInflections(
        AbsoluteCompositionPattern pattern,
        AngleUnit angleUnit) => pattern.Function == "cos"
        ?
        [
            PeriodicFeature(
                pattern,
    ...
    private static ImmutableArray<Asymptote> AbsoluteHorizontalAsymptotes(
        AbsoluteCompositionPattern pattern)
    {
        ...
        ExactReal value = pattern.Form == AbsoluteCompositionForm.AbsoluteOfAffineFunction
            ? pattern.Scale.Value
            : Rational(BigRational.One);
        return
        [
    ...
    private static ImmutableArray<MonotoneRegion> AbsoluteOuterMonotonicity(
        AbsoluteCompositionPattern pattern,
        AngleUnit angleUnit)
    {
        ...
        RealSet first = PeriodicInterval(
            pattern,
            angleUnit,
            0,
            new BigRational(1, 2),
            period,
            IntegerConstraint.All(Parameter));
        RealSet second = PeriodicInterval(
            pattern,
            angleUnit,
            new BigRational(1, 2),
            1,
        ...
        [
            new MonotoneRegion(
                first,
                sine ? Monotonicity.Increasing : Monotonicity.Decreasing),
            new MonotoneRegion(
                second,
                sine ? Monotonicity.Decreasing : Monotonicity.Increasing)
        ];
    }

    private static ImmutableArray<MonotoneRegion> AbsoluteInnerMonotonicity(
        AbsoluteCompositionPattern pattern,
        AngleUnit angleUnit)
    {
        if (pattern.Function != "cos")
    ...
    private static ImmutableArray<MonotoneRegion> CenteredMonotonicity(
        AbsoluteCompositionPattern pattern)
    {
    ...
    private static Periodicity AbsoluteOuterPeriod(
        AbsoluteCompositionPattern pattern,
        AngleUnit angleUnit) => pattern.Function is "sin" or "cos"
        ? new Periodicity(
            PeriodicityKind.PeriodicWithFundamentalPeriod,
            TrigStep(pattern, angleUnit, 1))
        : new Periodicity(PeriodicityKind.NotPeriodic, null);

    private static Periodicity AbsoluteInnerPeriod(
        AbsoluteCompositionPattern pattern,
        AngleUnit angleUnit) => pattern.Function == "cos"
        ? new Periodicity(
            PeriodicityKind.PeriodicWithFundamentalPeriod,
            TrigStep(pattern, angleUnit, 2))
        : new Periodicity(PeriodicityKind.NotPeriodic, null);

    private static ImmutableArray<FeaturePoint> SineAbsoluteMinima(
        AbsoluteCompositionPattern pattern,
        AngleUnit angleUnit)
    {
        ExactReal zero = Rational(BigRational.Zero);
        ...
        return
        [
            SingletonFeature(Center(pattern), zero),
            PeriodicFeature(
                pattern,
                angleUnit,
                new BigRational(3, 2),
                2,
                Constraint(Comparison.GreaterOrEqual),
                minusOne),
            PeriodicFeature(
                pattern,
                angleUnit,
                new BigRational(-3, 2),
                2,
                Constraint(Comparison.LessOrEqual),
                minusOne)
        ];
    }

    private static ImmutableArray<FeaturePoint> SineAbsoluteMaxima(
        AbsoluteCompositionPattern pattern,
        AngleUnit angleUnit)
    {
    ...
    private static ImmutableArray<FeaturePoint> SineAbsoluteInflections(
        AbsoluteCompositionPattern pattern,
        AngleUnit angleUnit)
    {
        ExactReal zero = Rational(BigRational.Zero);
    ...
    private static ImmutableArray<MonotoneRegion> SineAbsoluteMonotonicity(
        AbsoluteCompositionPattern pattern,
        AngleUnit angleUnit)
    {
        ExactReal center = Center(pattern);
    ...
    private static ConstantYFeaturePoint PeriodicFeature(
        ...
        new(
            new PeriodicReal(
                ...
                TrigStep(pattern, angleUnit, periodFraction),
                Parameter,
                constraint),
            y);

    private static Graphing.Symbolics.PeriodicPointSet TrigPointSet(
        AbsoluteCompositionPattern pattern,
        AngleUnit angleUnit,
        BigRational fraction,
        BigRational periodFraction) =>
    ...
    private static Graphing.Symbolics.RationalReal Center(AbsoluteCompositionPattern pattern) =>
        Rational(-pattern.Intercept / pattern.Slope);

    private static ExactReal SolveAngle(
        AbsoluteCompositionPattern pattern,
        ExactReal angle) =>
        ExactRealArithmetic.Scale(
            ExactRealArithmetic.Subtract(angle, Rational(pattern.Intercept)),
            pattern.Slope.Reciprocal());

    private static ExactReal TrigStep(
        AbsoluteCompositionPattern pattern,
        AngleUnit angleUnit,
        BigRational fraction) =>
        ExactRealArithmetic.Scale(
    ...
    private static ExactReal Angle(AngleUnit unit, BigRational piFraction) => unit switch
    {
        ...
        AngleUnit.Degrees => Rational(new BigRational(180) * piFraction),
        AngleUnit.Grads => Rational(new BigRational(200) * piFraction),
