...
internal static class PolynomialFormulaConverter
{
    ...
    public static bool Evaluate(PolynomialFormula formula, IReadOnlyDictionary<string, int> signs)
    {
        return formula switch
        {
            ...
            PolynomialJunction { IsConjunction: true } conjunction => conjunction.Operands.All(operand =>
                Evaluate(operand, signs)),
    ...
    public static ImmutableArray<UnivariatePolynomial> Atoms(PolynomialFormula formula)
    {
        return formula switch
        {
            PolynomialAtom atom => [atom.Polynomial],
            PolynomialNot not => Atoms(not.Operand),
            PolynomialJunction junction => junction.Operands.SelectMany(static operand => Atoms(operand))
                .DistinctBy(static polynomial => polynomial.Canonical)
                .OrderBy(static polynomial => polynomial.Canonical, StringComparer.Ordinal).ToImmutableArray(),
            _ => []
        };
    }
