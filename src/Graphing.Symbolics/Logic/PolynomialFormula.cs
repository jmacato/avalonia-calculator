...
internal sealed record PolynomialAtom(
    UnivariatePolynomial Polynomial,
    Comparison Comparison) : PolynomialFormula
{
...
internal sealed record PolynomialJunction(
    bool IsConjunction,
    ImmutableArray<PolynomialFormula> Operands) : PolynomialFormula
{
    public override string Canonical =>
        $"{(IsConjunction ? "and" : "or")}[{string.Join(',', Operands.Select(static operand => operand.Canonical))}]";
}
...
internal static class PolynomialFormulaConverter
{
        ...
        switch (formula)
        {
            ...
            case ComparisonFormula comparison:
                if (!RationalFunctionExtractor.TryExtract(
                        ...
                        variable,
                        budget,
                        out RationalExtraction left) ||
                    !RationalFunctionExtractor.TryExtract(
                        comparison.Right,
                        variable,
                        budget,
                        out RationalExtraction right))
                {
                    break;
                }

                RationalFunction difference = left.Function.Subtract(right.Function, budget);
                UnivariatePolynomial signPolynomial = difference.Numerator.Multiply(
                ...
                Comparison normalizedComparison = signPolynomial.LeadingCoefficient.Sign < 0
                    ? Reverse(comparison.Comparison)
                    : comparison.Comparison;
                result = new PolynomialAtom(
                    signPolynomial.PrimitivePositive(budget),
                    normalizedComparison);
                return true;
            case PredicateFormula predicate when
                predicate.Terms.Length == 1 &&
                predicate.Terms[0].Kind == ValueKind.Constant:
                BigRational value = predicate.Terms[0].Constant;
                result = predicate.Kind switch
                {
                    ExactPredicate.IsInteger => new PolynomialBoolean(value.IsInteger),
                    ExactPredicate.IsOddInteger => new PolynomialBoolean(
    ...
    public static bool Evaluate(
        ...
        IReadOnlyDictionary<string, int> signs) => formula switch
    {
        ...
        PolynomialJunction { IsConjunction: true } conjunction =>
            conjunction.Operands.All(operand => Evaluate(operand, signs)),
        PolynomialJunction disjunction =>
            disjunction.Operands.Any(operand => Evaluate(operand, signs)),
    ...
    public static ImmutableArray<UnivariatePolynomial> Atoms(PolynomialFormula formula) => formula switch
    {
        ...
        PolynomialJunction junction => junction.Operands
            .SelectMany(static operand => Atoms(operand))
