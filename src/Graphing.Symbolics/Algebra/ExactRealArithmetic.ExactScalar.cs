...
{
    public static ExactReal Negate(ExactReal value) => value switch
    {
    ...
    public static ExactReal Add(ExactReal left, ExactReal right) => (left, right) switch
    {
        ...
        (AffinePiReal a, RationalReal b) => a with
        {
            Constant = a.Constant + b.Value
        },
        (RationalReal a, AffinePiReal b) => b with
        {
            Constant = b.Constant + a.Value
        },
    ...
    public static ExactReal AddRational(ExactReal value, BigRational addend) => Add(value, new RationalReal(addend));
    public static ExactReal Subtract(ExactReal left, ExactReal right) => Add(left, Negate(right));
    public static ExactReal Multiply(ExactReal left, ExactReal right) => (left, right) switch
    {
        (RationalReal a, _) => Scale(right, a.Value),
        (_, RationalReal b) => Scale(left, b.Value),
        _ => new FunctionReal("multiply", [left, right])
    };
    public static ExactReal Divide(ExactReal numerator, ExactReal denominator) => denominator is RationalReal rational ? Scale(numerator, rational.Value.Reciprocal()) : new FunctionReal("divide", [numerator, denominator]);
    public static ExactReal Power(ExactReal basis, int exponent) => new FunctionReal("power", [basis, new RationalReal(new BigRational(exponent))]);
