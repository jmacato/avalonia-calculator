...
internal static class ExactRationalExtractor
{
    ...
    private static ImmutableArray<ExactCoefficientPolynomial> DistinctExclusions(IEnumerable<ExactCoefficientPolynomial> exclusions) => exclusions.Where(static exclusion => !exclusion.IsConstant || exclusion[0].IsZero).DistinctBy(static exclusion => exclusion.Canonical).OrderBy(static exclusion => exclusion.Canonical, StringComparer.Ordinal).ToImmutableArray();
