...
internal static class ExactScalarOrder
{
    ...
    private static bool TryEncloseFunction(FunctionReal function, ResourceBudget budget, out RationalEnclosure enclosure) => function.Function switch
    {
    ...
    private static RationalEnclosure Exact(BigRational value, ResourceBudget budget) => Checked(new RationalEnclosure(value, value), budget);
    private static RationalEnclosure Add(RationalEnclosure left, RationalEnclosure right, ResourceBudget budget) => Checked(new RationalEnclosure(left.Lower + right.Lower, left.Upper + right.Upper), budget);
    private static RationalEnclosure Negate(RationalEnclosure value, ResourceBudget budget) => Checked(new RationalEnclosure(-value.Upper, -value.Lower), budget);
    private static RationalEnclosure Scale(RationalEnclosure value, BigRational factor, ResourceBudget budget) => factor.Sign < 0 ? Checked(new RationalEnclosure(value.Upper * factor, value.Lower * factor), budget) : Checked(new RationalEnclosure(value.Lower * factor, value.Upper * factor), budget);
    ...
    private static BigRational Min(BigRational left, BigRational right) => left <= right ? left : right;
    private static BigRational Max(BigRational left, BigRational right) => left >= right ? left : right;
}
