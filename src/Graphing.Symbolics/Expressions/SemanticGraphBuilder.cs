...
internal sealed class SemanticGraphBuilder(ResourceBudget budget)
{
    ...
    private static SemanticExpression Leaf(ValueTerm value, SourceRange source)
    {
        return new SemanticExpression(value, Formula.True, Formula.True, Formula.True, [source], [], []);
    }
    ...
    {
        Formula nonzero = ExactScalar.TryCreate(denominator.Value, budget, out ExactScalar scalarDenominator) ? scalarDenominator.IsZero ? Formula.False : Formula.True : Formula.Compare(denominator.Value, Comparison.NotEqual, Constant(BigRational.Zero));
    ...
    private static SemanticExpression IdentityFunction(SemanticExpression operand, SourceRange source)
    {
        return new SemanticExpression(operand.Value, operand.DefinedWhen, operand.ContinuousWhen, operand.DifferentiableWhen,
            MergeProvenance(source, [operand]), [], [operand]);
    }
    ...
    private ValueTerm FoldBinary(ValueKind kind, ValueTerm left, ValueTerm right, out RewriteStep? rewrite)
    {
        ...
        if (left.Kind == ValueKind.Constant && right.Kind == ValueKind.Constant)
        {
            ...
            {
                budget.CheckCoefficient(folded.Value);
    ...
    private bool TryFoldExactIdentity(ValueKind kind, ValueTerm left, ValueTerm right, out ValueTerm value, out RewriteStep? rewrite)
    {
        ...
        if (kind is ValueKind.Add or ValueKind.Subtract && ExactScalar.TryCreate(left, budget, out ExactScalar exactLeft) && ExactScalar.TryCreate(right, budget, out ExactScalar exactRight) && ExactScalar.TryAdd(exactLeft, kind == ValueKind.Add ? exactRight : exactRight.Negate(), budget, out ExactScalar exactSum) && exactSum.RationalValue is { } rationalSum)
        {
    ...
    {
        return Term(ValueKind.Constant, value, string.Empty, []);
    }
    ...
    private static ImmutableArray<SourceRange> MergeProvenance(SourceRange source, IEnumerable<SemanticExpression> operands)
    {
        return operands.SelectMany(static operand => operand.Provenance).Append(source).Distinct()
            .OrderBy(static value => value.Start).ThenBy(static value => value.Length).ToImmutableArray();
    }
}
