...
internal sealed record CompiledGraphEquation(uint EquationId, GraphEquationKind Kind, RelationKind Relation, ExpressionProgram BoundaryProgram, ExpressionProgram? RegionProgram, AstNode BoundarySyntax, string Source, int XIndex, int YIndex, SourceSpan Span)
{
    ...
    {
        return node.Kind == AstKind.Variable && string.Equals(node.Name, name, StringComparison.OrdinalIgnoreCase);
    }
    ...
    {
        return new AstNode(AstKind.Subtract, span, 0, string.Empty, [left, right]);
    }

    private static GraphParseException Unsupported(EquationAst equation, string message)
    {
        return new GraphParseException(SyntaxErrorCode.InvalidEquationSyntax, equation.Span, message);
    }
}
