...
internal sealed class ManagedGraphRenderer : IGraphRenderer, IConcurrentGraphRenderer, IDisposable
{
    ...
    private static bool IsAffineBoundary(AstNode node) => node.Kind switch
    {
        ...
        AstKind.Multiply => IsConstantBoundary(node.Children[0]) && IsAffineBoundary(node.Children[1]) || IsAffineBoundary(node.Children[0]) && IsConstantBoundary(node.Children[1]),
        AstKind.Divide => IsAffineBoundary(node.Children[0]) && IsConstantBoundary(node.Children[1]),
        _ => false
    };
    private static bool IsConstantBoundary(AstNode node) => node.Kind switch
    {
        AstKind.Number => true,
        AstKind.Negate => IsConstantBoundary(node.Children[0]),
        AstKind.Add or AstKind.Subtract or AstKind.Multiply or AstKind.Divide => IsConstantBoundary(node.Children[0]) && IsConstantBoundary(node.Children[1]),
        _ => false
    };
    private static bool IsInequalitySatisfied(RelationKind relation, double value) => double.IsFinite(value) && relation switch
    {
