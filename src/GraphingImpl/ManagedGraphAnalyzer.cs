...
internal sealed class ManagedGraphAnalyzer : ICancellableGraphAnalyzer
{
    ...
    {
        return PerformFunctionAnalysis(analysisType, CancellationToken.None);
    }
    ...
    private static ImmutableArray<string> FormatPoints(ProofOutcome<ImmutableArray<FeaturePoint>> outcome)
    {
        return Proved(outcome, out ImmutableArray<FeaturePoint> points)
    ...
    private static ImmutableArray<string> FormatAsymptotes(ProofOutcome<ImmutableArray<Asymptote>> outcome)
    {
        return Proved(outcome, out ImmutableArray<Asymptote> asymptotes)
