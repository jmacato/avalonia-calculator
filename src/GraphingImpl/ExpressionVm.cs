...
internal readonly record struct EvaluationValue(double Value, EvaluationState State)
{
    ...
    public static EvaluationValue Finite(double value) => double.IsFinite(value)
...
internal readonly record struct DerivativeJet(
    double Value,
    double First,
    double Second,
    EvaluationState State)
{
    public bool IsFinite =>
        State == EvaluationState.Finite &&
        double.IsFinite(Value) &&
        double.IsFinite(First) &&
        double.IsFinite(Second);

    public static DerivativeJet Constant(double value) => double.IsFinite(value)
    ...
    public static DerivativeJet Invalid(EvaluationState state) =>
        new(double.NaN, double.NaN, double.NaN, state);
}

internal readonly record struct ConservativeInterval(
    double Minimum,
    double Maximum,
    EvaluationState State)
{
    ...
    public static ConservativeInterval Point(double value) => double.IsFinite(value)
        ? new(value, value, EvaluationState.Finite)
        : Invalid(double.IsInfinity(value) ? EvaluationState.Overflow : EvaluationState.Undefined);

    public static ConservativeInterval Invalid(EvaluationState state) =>
        new(double.NaN, double.NaN, state);
}
...
internal readonly record struct VmInstruction(
    VmOpCode OpCode,
    double Constant = 0,
    int Operand = 0);
...
internal sealed class ExpressionProgram
{
    ...
    private ExpressionProgram(
        ImmutableArray<VmInstruction> instructions,
        ...
        int operationBudget = 16_384)
    {
        ...
        Span<EvaluationValue> stack = MaximumStack <= 128
            ? stackalloc EvaluationValue[MaximumStack]
            : (rented = ArrayPool<EvaluationValue>.Shared.Rent(MaximumStack));
    ...
    public DerivativeJet EvaluateJet(
        ReadOnlySpan<double> variables,
        int differentiationVariable,
        EvalTrigUnitMode trigMode,
        int operationBudget = 16_384)
    {
        ...
        Span<DerivativeJet> stack = MaximumStack <= 128
            ? stackalloc DerivativeJet[MaximumStack]
            : (rented = ArrayPool<DerivativeJet>.Shared.Rent(MaximumStack));
    ...
    public ConservativeInterval EvaluateInterval(
        ReadOnlySpan<ConservativeInterval> variables,
        EvalTrigUnitMode trigMode,
        int operationBudget = 16_384)
    {
        if (variables.Length < Symbols.Length)
        ...
        Span<ConservativeInterval> stack = MaximumStack <= 128
            ? stackalloc ConservativeInterval[MaximumStack]
            : (rented = ArrayPool<ConservativeInterval>.Shared.Rent(MaximumStack));
    ...
    private static EvaluationValue ApplyScalarFunction(
        VmOpCode operation,
        EvaluationValue input,
        EvalTrigUnitMode trigMode)
    {
        if (!input.IsFinite)
        {
    ...
    private static void BinaryScalar(
        Span<EvaluationValue> stack,
        ref int top,
        Func<double, double, EvaluationValue> operation)
    {
        EvaluationValue right = stack[--top];
        EvaluationValue left = stack[top - 1];
        stack[top - 1] = !left.IsFinite
            ? left
            : !right.IsFinite
                ? right
    ...
    private static void AggregateScalar(
        Span<EvaluationValue> stack,
        ref int top,
        int count,
        bool product,
        ...
        bool maximum)
    {
        ...
        for (int index = start; index < top; index++)
        {
            ...
            result = product
                ? result * item.Value
                : minimum
                    ? Math.Min(result, item.Value)
                    : maximum
                        ? Math.Max(result, item.Value)
                        : result + item.Value;
        }

        top = start + 1;
        stack[start] = state == EvaluationState.Finite
            ? EvaluationValue.Finite(result)
            : EvaluationValue.Invalid(state);
    }

    private static EvaluationValue Unary(EvaluationValue input, Func<double, double> operation) =>
        input.IsFinite ? EvaluationValue.Finite(operation(input.Value)) : input;
    ...
    private static void ExecuteJet(
        VmInstruction instruction,
        Span<DerivativeJet> stack,
        ref int top,
        ReadOnlySpan<double> variables,
        int differentiationVariable,
        EvalTrigUnitMode trigMode)
    {
        switch (instruction.OpCode)
        {
            case VmOpCode.Constant:
                ...
                return;
            case VmOpCode.Variable:
                stack[top++] = new DerivativeJet(
                    variables[instruction.Operand],
                    instruction.Operand == differentiationVariable ? 1 : 0,
                    0,
                    double.IsFinite(variables[instruction.Operand])
                        ? EvaluationState.Finite
    ...
    private static DerivativeJet ApplyJetFunction(
        VmOpCode operation,
        DerivativeJet input,
        EvalTrigUnitMode trigMode)
    {
        ...
        {
            case VmOpCode.Sin:
                return Compose(input, Math.Sin(x * radians), radians * Math.Cos(x * radians),
                    -radians * radians * Math.Sin(x * radians));
            case VmOpCode.Cos:
                return Compose(input, Math.Cos(x * radians), -radians * Math.Sin(x * radians),
                    -radians * radians * Math.Cos(x * radians));
            case VmOpCode.Tan:
            {
                double tangent = Math.Tan(x * radians);
                double secantSquared = 1 + (tangent * tangent);
                return Compose(input, tangent, radians * secantSquared,
                    2 * radians * radians * secantSquared * tangent);
            }
            case VmOpCode.Cot:
            case VmOpCode.Sec:
            case VmOpCode.Csc:
            ...
            case VmOpCode.Asin when x is >= -1 and <= 1:
            {
                ...
                return Compose(input, Math.Asin(x) / radians, 1 / (radians * denominator),
                    x / (radians * denominator * denominator * denominator));
            }
            case VmOpCode.Acos when x is >= -1 and <= 1:
            {
                double denominator = Math.Sqrt(1 - (x * x));
                return Compose(input, Math.Acos(x) / radians, -1 / (radians * denominator),
                    -x / (radians * denominator * denominator * denominator));
            }
            case VmOpCode.Atan:
            {
                double denominator = 1 + (x * x);
                return Compose(input, Math.Atan(x) / radians, 1 / (radians * denominator),
                    -2 * x / (radians * denominator * denominator));
            }
    ...
    private static DerivativeJet ApplyNonSmoothJet(VmOpCode operation, DerivativeJet input)
    {
        ...
        if (operation == VmOpCode.Factorial && input.First == 0 && input.Second == 0)
        {
            ...
            return factorial.IsFinite
                ? DerivativeJet.Constant(factorial.Value)
                : DerivativeJet.Invalid(factorial.State);
        }

        return DerivativeJet.Invalid(EvaluationState.NonReal);
    }

    private static DerivativeJet NumericalUnaryJet(
        VmOpCode operation,
        DerivativeJet input,
        EvalTrigUnitMode trigMode)
    {
    ...
    {
        DerivativeJet result = new(
            value,
            first * input.First,
            (second * input.First * input.First) + (first * input.Second),
            EvaluationState.Finite);
        return result.IsFinite ? result : DerivativeJet.Invalid(EvaluationState.Overflow);
    }

    private static DerivativeJet Negate(DerivativeJet value) => value.IsFinite
        ? new(-value.Value, -value.First, -value.Second, EvaluationState.Finite)
        : value;

    private static DerivativeJet Add(DerivativeJet left, DerivativeJet right) =>
        FinishJet(left.Value + right.Value, left.First + right.First, left.Second + right.Second);

    private static DerivativeJet Subtract(DerivativeJet left, DerivativeJet right) =>
        FinishJet(left.Value - right.Value, left.First - right.First, left.Second - right.Second);

    private static DerivativeJet Multiply(DerivativeJet left, DerivativeJet right) => FinishJet(
    ...
    private static DerivativeJet DivideJet(DerivativeJet left, DerivativeJet right)
    {
        ...
        var reciprocal = FinishJet(
            reciprocalValue,
            -right.First / (right.Value * right.Value),
            ((2 * right.First * right.First) - (right.Value * right.Second)) /
    ...
    private static DerivativeJet PowerJet(DerivativeJet basis, DerivativeJet exponent)
    {
        if (basis.Value > 0)
        {
            DerivativeJet logarithm = Compose(basis, Math.Log(basis.Value), 1 / basis.Value,
                -1 / (basis.Value * basis.Value));
        ...
        if (exponent.First == 0 && exponent.Second == 0 && IsInteger(exponent.Value))
        {
            ...
            return Compose(
                basis,
                value,
                n * Math.Pow(basis.Value, n - 1),
                n * (n - 1) * Math.Pow(basis.Value, n - 2));
        }
    ...
    private static void BinaryJet(
        Span<DerivativeJet> stack,
        ref int top,
        Func<DerivativeJet, DerivativeJet, DerivativeJet> operation)
    {
    ...
    {
        int start = top - count;
        DerivativeJet result = operation == VmOpCode.Product
            ? DerivativeJet.Constant(1)
            : stack[start];
    ...
    private static DerivativeJet FinishJet(double value, double first, double second) =>
        double.IsFinite(value) && double.IsFinite(first) && double.IsFinite(second)
    ...
    private static void ExecuteInterval(
        VmInstruction instruction,
        Span<ConservativeInterval> stack,
        ref int top,
        ReadOnlySpan<ConservativeInterval> variables,
        EvalTrigUnitMode trigMode)
    {
        switch (instruction.OpCode)
        {
            ...
            case VmOpCode.Negate:
            {
                ...
                stack[top - 1] = value.IsFinite
                    ? new ConservativeInterval(-value.Maximum, -value.Minimum, EvaluationState.Finite)
                    : value;
                return;
            }
            case VmOpCode.Add:
    ...
    private static ConservativeInterval ApplyIntervalFunction(
        VmOpCode operation,
        ConservativeInterval input,
        EvalTrigUnitMode trigMode)
    {
        if (!input.IsFinite)
        {
        ...
        return operation switch
        {
            ...
            VmOpCode.Tan when CrossesTangentPole(input.Minimum * radians, input.Maximum * radians) =>
                ConservativeInterval.Invalid(EvaluationState.Undefined),
            VmOpCode.Tan => Ordered(Math.Tan(input.Minimum * radians), Math.Tan(input.Maximum * radians)),
            VmOpCode.Asin when input.Minimum >= -1 && input.Maximum <= 1 =>
                Ordered(Math.Asin(input.Minimum) / radians, Math.Asin(input.Maximum) / radians),
            VmOpCode.Acos when input.Minimum >= -1 && input.Maximum <= 1 =>
                Ordered(Math.Acos(input.Maximum) / radians, Math.Acos(input.Minimum) / radians),
            ...
            VmOpCode.Factorial when input.Minimum == input.Maximum =>
                ToInterval(Factorial(input.Minimum)),
    ...
    private static ConservativeInterval AddInterval(ConservativeInterval left, ConservativeInterval right) =>
        Ordered(left.Minimum + right.Minimum, left.Maximum + right.Maximum);

    private static ConservativeInterval SubtractInterval(ConservativeInterval left, ConservativeInterval right) =>
        Ordered(left.Minimum - right.Maximum, left.Maximum - right.Minimum);
    ...
    private static ConservativeInterval PowerInterval(ConservativeInterval basis, ConservativeInterval exponent)
    {
        ...
        {
            return basis.Minimum >= 0 && exponent.Minimum == exponent.Maximum
                ? Ordered(Math.Pow(basis.Minimum, exponent.Minimum), Math.Pow(basis.Maximum, exponent.Maximum))
                : ConservativeInterval.Invalid(EvaluationState.NonReal);
        }
    ...
    private static ConservativeInterval RootInterval(ConservativeInterval radicand, ConservativeInterval index)
    {
        ...
        return left.IsFinite && right.IsFinite
            ...
            : ConservativeInterval.Invalid(!left.IsFinite ? left.State : right.State);
    }

    private static void BinaryInterval(
        Span<ConservativeInterval> stack,
        ref int top,
        Func<ConservativeInterval, ConservativeInterval, ConservativeInterval> operation)
    {
    ...
    private static void AggregateInterval(
        Span<ConservativeInterval> stack,
        ref int top,
        VmOpCode operation,
        int count)
    {
        int start = top - count;
        ConservativeInterval result = operation == VmOpCode.Product
            ? ConservativeInterval.Point(1)
            : stack[start];
        int index = operation == VmOpCode.Product ? start : start + 1;
        for (; index < top; index++)
        {
            ...
            result = operation switch
            {
                ...
                VmOpCode.Minimum => Ordered(
                    Math.Min(result.Minimum, item.Minimum),
                    Math.Min(result.Maximum, item.Maximum)),
                VmOpCode.Maximum => Ordered(
                    Math.Max(result.Minimum, item.Minimum),
                    Math.Max(result.Maximum, item.Maximum)),
                _ => ConservativeInterval.Invalid(EvaluationState.Undefined)
            };
        }
    ...
    private static ConservativeInterval CoshInterval(ConservativeInterval input)
    {
        ...
        return input.Minimum <= 0 && input.Maximum >= 0
            ? Ordered(1, Math.Max(left, right))
            : Ordered(left, right);
    }
    ...
    private static ConservativeInterval Ordered(double first, double second)
    {
        ...
        return double.IsFinite(minimum) && double.IsFinite(maximum)
            ? new ConservativeInterval(minimum, maximum, EvaluationState.Finite)
            : ConservativeInterval.Invalid(double.IsInfinity(minimum) || double.IsInfinity(maximum)
                ? EvaluationState.Overflow
                : EvaluationState.Undefined);
    }

    private static ConservativeInterval ToInterval(EvaluationValue value) => value.IsFinite
        ? ConservativeInterval.Point(value.Value)
        : ConservativeInterval.Invalid(value.State);

    private static double RadiansFactor(EvalTrigUnitMode mode) => mode switch
    {
    ...
    private static bool IsInteger(double value) =>
        double.IsFinite(value) && value == Math.Truncate(value) && Math.Abs(value) <= 9_007_199_254_740_992d;
    ...
    private sealed class Compiler
    {
        ...
        public Compiler(ImmutableArray<string> symbols)
        {
        ...
        private static void ValidateArity(AstNode node, VmOpCode operation)
        {
            ...
            if (!valid)
            {
                throw new GraphParseException(
                    ...
                    node.Span,
                    $"Function '{node.Name}' received an incorrect number of arguments.");
            }
        }
