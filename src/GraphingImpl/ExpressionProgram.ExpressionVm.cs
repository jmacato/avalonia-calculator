...
internal sealed class ExpressionProgram
{
    ...
    private static EvaluationValue Unary(EvaluationValue input, Func<double, double> operation) => input.IsFinite ? EvaluationValue.Finite(operation(input.Value)) : input;
    private static EvaluationValue Divide(double numerator, double denominator) => denominator == 0 ? EvaluationValue.Invalid(EvaluationState.Undefined) : EvaluationValue.Finite(numerator / denominator);
    ...
    private static DerivativeJet Negate(DerivativeJet value) => value.IsFinite ? new(-value.Value, -value.First, -value.Second, EvaluationState.Finite) : value;
    private static DerivativeJet Add(DerivativeJet left, DerivativeJet right) => FinishJet(left.Value + right.Value, left.First + right.First, left.Second + right.Second);
    private static DerivativeJet Subtract(DerivativeJet left, DerivativeJet right) => FinishJet(left.Value - right.Value, left.First - right.First, left.Second - right.Second);
    private static DerivativeJet Multiply(DerivativeJet left, DerivativeJet right) => FinishJet(left.Value * right.Value, (left.First * right.Value) + (left.Value * right.First), (left.Second * right.Value) + (2 * left.First * right.First) + (left.Value * right.Second));
    ...
    private static DerivativeJet FinishJet(double value, double first, double second) => double.IsFinite(value) && double.IsFinite(first) && double.IsFinite(second) ? new DerivativeJet(value, first, second, EvaluationState.Finite) : DerivativeJet.Invalid(EvaluationState.Overflow);
    ...
    private static ConservativeInterval AddInterval(ConservativeInterval left, ConservativeInterval right) => Ordered(left.Minimum + right.Minimum, left.Maximum + right.Maximum);
    private static ConservativeInterval SubtractInterval(ConservativeInterval left, ConservativeInterval right) => Ordered(left.Minimum - right.Maximum, left.Maximum - right.Minimum);
    ...
    private static ConservativeInterval ToInterval(EvaluationValue value) => value.IsFinite ? ConservativeInterval.Point(value.Value) : ConservativeInterval.Invalid(value.State);
    ...
    private static bool IsInteger(double value) => double.IsFinite(value) && value == Math.Truncate(value) && Math.Abs(value) <= 9_007_199_254_740_992d;
}
