...
standard C++. That ad hoc implementation has some ad hoc limitations. In
...
`ADD_FAILURE*`) in constructors and destructors. The alternative is to move the
...
If you really need to see such LOG messages, a alternative is to temporarily
break the death test (e.g. by changing the regex pattern it is expected to
match). Admittedly, this is a implementation technique. We'll consider a more permanent solution
...
*horrible* implementation technique of sniffing your executable name (`argv[0]` in `main()`) to know
