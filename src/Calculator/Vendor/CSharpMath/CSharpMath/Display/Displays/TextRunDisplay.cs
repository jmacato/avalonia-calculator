...
public class TextRunDisplay<TFont, TGlyph> : IDisplay<TFont, TGlyph> where TFont : IFont<TGlyph> {
    public TextRunDisplay(
        ...
        Ascent = rectangleFs.IsEmpty() ? 0 : rectangleFs.Max(rect => rect.Bottom); // Convert to non-flipped naming here,
        Descent = rectangleFs.IsEmpty() ? 0 : rectangleFs.Max(rect => -rect.Y);
    }
