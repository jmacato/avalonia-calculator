...
public static class UnicodeFontChanger {
    ...
    private static bool IsLowerEn(char c) => c is >= 'a' and <= 'z';
    private static bool IsUpperEn(char c) => c is >= 'A' and <= 'Z';
    private static bool IsNumber(char c) => c is >= '0' and <= '9';
    private static bool IsLowerGreek(char c) => c is >= UnicodeGreekLowerStart and <= UnicodeGreekLowerEnd;
    private static bool IsUpperGreek(char c) => c is >= UnicodeGreekUpperStart and <= UnicodeGreekUpperEnd;

    private static readonly char[] GreekSymbols = {
        'ϵ', // 03F5
        'ϑ', // 03D1
        'ϰ', // 03F0
        'ϕ', // 03D5
    ...
    private static int GetBlackboard(char c) =>
        ...
        c switch
        {
            ...
            'R' => 0x211D, // Reals
            'Z' => 0x2124, // Integers
            _ when IsUpperEn(c) => UnicodeMathCapitalBlackboardStart + c - 'A',
            _ when IsLowerEn(c) => UnicodeMathLowerBlackboardStart + c - 'a',
            _ when IsNumber(c) => UnicodeNumberBlackboardStart + c - '0',
            _ => GetDefaultStyle(c),
        };
