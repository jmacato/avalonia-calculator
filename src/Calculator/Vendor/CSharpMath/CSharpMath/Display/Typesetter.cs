    ...
    private static TGlyph FindVariantGlyph<TFont, TGlyph>(FontMathTable<TFont, TGlyph> mathTable,
        ...
        foreach (var (advance, bounds, glyph) in advances.Zip(boundingBoxes, glyphsArray, ValueTuple.Create)) {
            ...
            if (width > targetWidth) {
                if (glyphAscent is not float.NaN) return glyph;
...
public class Typesetter<TFont, TGlyph> where TFont : IFont<TGlyph> {
    ...
    private LineStyle ScriptStyle => Style switch
    {
        ...
        _ => throw new
            System.ComponentModel.InvalidEnumArgumentException(nameof(Style), (int)Style, typeof(LineStyle))
    };
    ...
    private float SuperscriptShiftUp =>
        SuperscriptCramped
            ? _mathTable.SuperscriptShiftUpCramped(_styleFont)
            : _mathTable.SuperscriptShiftUp(_styleFont);

    private Typesetter(TFont font, TypesettingContext<TFont, TGlyph> context,
        ...
        foreach (var atom in preprocessedAtoms) {
            switch (atom) {
                ...
                case Style style:
                    ...
                    _styleFont =
                        _context.MathFontCloner.Invoke(_font, _mathTable.GetStyleSize(Style, _font));
                ...
                case Colored colored:
                    ...
                    var colorDisplay = CreateLine(colored.InnerList, _font, _context, Style, false);
                ...
                case ColorBox colorBox:
                    ...
                    colorDisplay = CreateLine(colorBox.InnerList, _font, _context, Style, false);
                ...
                case Inner inner:
                    ...
                    if (inner.LeftBoundary != Boundary.Empty || inner.RightBoundary != Boundary.Empty) {
                        ...
                        innerDisplay = CreateLine(inner.InnerList, _font, _context, Style, SuperscriptCramped);
                    }
                ...
                case Underline underline:
                    ...
                    var innerListDisplay = Typesetter<TFont, TGlyph>.CreateLine
                        (underline.InnerList, _font, _context, Style, SuperscriptCramped);
                ...
                case Overline overline:
                    ...
                    innerListDisplay = Typesetter<TFont, TGlyph>.CreateLine
                        (overline.InnerList, _font, _context, Style, true);
                    var overlineDisplay =
                        new OverOrUnderlineDisplay<TFont, TGlyph>(innerListDisplay, _currentPosition) {
                ...
                case RaiseBox raiseBox:
                    AddDisplayLine(false);
                    var raisedDisplay =
                        CreateLine(raiseBox.InnerList, _font, _context, Style, false);
                ...
                case Prime _: {
                    if (prevAtom != null) {
                        float interElementSpace =
                            InterElementSpaces.Get(prevAtom, atom, Style, _styleFont, _mathTable);
    ...
    private IDisplay<TFont, TGlyph> MakeAccent(Accent accent) {
        var accentee =
            CreateLine(accent.InnerList, _font, _context, Style, true);
        ...
        if (accent.InnerList.Atoms.Count == 1
            && accent.InnerList.Atoms[0] is { } innerAtom
    ...
    private void MakeScripts(MathAtom atom, IDisplay<TFont, TGlyph> display, int index, float delta) {
        ...
        if (atom.Superscript.IsEmpty()) {
            ...
            subscript.Position = _currentPosition with { Y = _currentPosition.Y - subscriptShiftDown };
        ...
        var superscript =
            CreateLine(atom.Superscript, _font, _context, ScriptStyle, SuperscriptCramped);
    ...
    private void AddInterElementSpace(MathAtom? prev, MathAtom current) =>
        _currentPosition.X +=
            prev != null ? InterElementSpaces.Get(prev, current, Style, _styleFont, _mathTable)
            : _spaced ? InterElementSpaces.Get(new Open(""), current, Style, _styleFont, _mathTable)
            : 0;

    private TextLineDisplay<TFont, TGlyph>? AddDisplayLine(bool evenIfLengthIsZero) {
        if (!evenIfLengthIsZero && (_currentLine.Length <= 0)) return null;
    ...
    private RadicalDisplay<TFont, TGlyph> MakeRadical(MathList radicand, Range range) {
        ...
        IGlyphDisplay<TFont, TGlyph> GetRadicalGlyph(float radicalHeight) {
            ...
            var findGlyph = FindGlyph(radicalGlyph, radicalHeight,
            ...
            return
                ...
                && ConstructGlyph(radicalGlyph, radicalHeight) is IGlyphDisplay<TFont, TGlyph> constructed
                    ...
                    : new GlyphDisplay<TFont, TGlyph>
                        (findGlyph, Range.NotFound, _styleFont, glyphAscent, glyphDescent, glyphWidth);
        }
    }
    ...
    private IDisplay<TFont, TGlyph> MakeFraction(Fraction fraction) {
        ...
        var denominatorDisplay =
            CreateLine(fraction.Denominator, _font, _context, FractionStyle, true);
    ...
    private InnerDisplay<TFont, TGlyph> MakeInner(Inner inner, Range range) {
        ...
        var leftGlyph =
            inner.LeftBoundary is { Nucleus: var left } && left?.Length > 0
                ? FindGlyphForBoundary(left, glyphHeight)
                : null;

        var rightGlyph =
            inner.RightBoundary is { Nucleus: var right } && right?.Length > 0
    ...
    private TGlyph FindGlyph(TGlyph rawGlyph, float height,
        ...
        var rects =
            _context.GlyphBoundsProvider.GetBoundingRectsForGlyphs(_styleFont, variantsArray, nVariants);
        var advances =
            _context.GlyphBoundsProvider.GetAdvancesForGlyphs(_styleFont, variantsArray, nVariants).Advances;
    ...
    private IDisplay<TFont, TGlyph> AddLimitsToDisplay(IDisplay<TFont, TGlyph> display,
        ...
        if (op.Limits ?? Style == LineStyle.Display) {
            ...
            if (op.Superscript.IsNonEmpty()) {
                superscript =
                    CreateLine(op.Superscript, _font, _context, ScriptStyle, SuperscriptCramped);
            }
            if (op.Subscript.IsNonEmpty()) {
                subscript =
                    CreateLine(op.Subscript, _font, _context, ScriptStyle, SubscriptCramped);
            }
