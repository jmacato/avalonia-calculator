...
using static InterElementSpacesInterElementSpaceType;

internal static class InterElementSpaces
{
    private static readonly InterElementSpacesInterElementSpaceType[][] Spaces =
    [
    ...
    public static float Get<TFont, TGlyph>(MathAtom left, MathAtom right, LineStyle style, TFont styleFont, FrontEnd.FontMathTable<TFont, TGlyph> mathTable)
        where TFont : FrontEnd.IFont<TGlyph>
    {
        ...
        var multiplier = Spaces[leftIndex][rightIndex] switch
        {
            Invalid => throw new Structures.InvalidCodePathException($"Invalid space between {left.TypeName} and {right.TypeName}"),
            None => 0,
            Thin => 3,
