...
public static partial class Extensions {
    ...
    public static float CollectionWidth<TFont, TGlyph>
        (this IEnumerable<IDisplay<TFont, TGlyph>> displays) where TFont : IFont<TGlyph> {
        var enumerable = displays as IDisplay<TFont, TGlyph>[] ?? displays.ToArray();
        return enumerable.IsNonEmpty()
            ? enumerable.Max(d => d.Position.X + d.Width) - enumerable.Min(d => d.Position.X)
            : 0;
    }
    ...
    private static RectangleF Plus(this RectangleF rect, PointF vector) =>
