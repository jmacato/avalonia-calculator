...
#pragma warning disable CA1034 // Nested types should not be visible
    ...
    public class TableEnvironment(string? name) : IEnvironment {
...
#pragma warning restore CA1034 // Nested types should not be visible
    ...
    public Stack<IEnvironment> Environments { get; } = new();
    ...
    public Result<Color> ReadColor() {
        ...
        if (LaTeXSettings.ParseColor(str) is not { } color) return "Invalid color: " + str;
        SkipSpaces();
        if (!ReadCharIfAvailable('}'))
            return "Missing }";
        return color;
    ...
    public Result<string> ReadEnvironment() {
        ...
        var env = ReadString();
        SkipSpaces();
        return !ReadCharIfAvailable('}') ? Err("Missing }") : Ok(env);
    }
    ...
    public Result<MathAtom> ReadTable
        ...
        if (environment is { Name: not null, Ended: false }) {
    ...
    private static void MathListToLaTeX
        ...
        static bool MathAtomToLaTeX(MathAtom atom, StringBuilder builder,
            ...
            if (LaTeXSettings.CommandForAtom(atom) is { } name) {
                command = name;
                builder.Append(name);
