...
{
    private readonly List<MathAtom> _atoms;
    public IList<MathAtom> Atoms => _atoms;

    public MathList() => _atoms = [];
    public MathList(IEnumerable<MathAtom> atoms) => _atoms = [.. atoms];
    public MathList(params MathAtom[] atoms) => _atoms = [.. atoms];
    /// <return s>The last <see cref = "MathAtom"/> that is not a <see cref = "Comment"/>,
    /// or <see cref = " null  "/> when <see cref = "Atoms"/> is empty.</return s>
    ...
    public MathList Clone(bool finalize)
    {
        ...
        else
        {
            foreach (var atom in Atoms)
            {
                ...
                {
                    int prevIndex = prevNode?.IndexRange.Location + prevNode?.IndexRange.Length ?? 0;
    ...
    public string DebugString => string.Concat(System.Linq.Enumerable.Select(Atoms, a => a.DebugString));
    public bool IsReadOnly => false;

    public MathAtom this[int index] { get => Atoms[index]; set => Atoms[index] = value; }

    public virtual void Append(IEnumerable<MathAtom> list) => _atoms.AddRange(list);
    public void RemoveAtoms(int index, int count) => _atoms.RemoveRange(index, count);
    ...
    public override int GetHashCode() => Atoms.Count == 0 ? 0 : Atoms.GetHashCode(); // Special case empty list for LaTeXDefaults
    ...
    public MathList Slice(int index, int count) => new(_atoms.GetRange(index, count));
}
