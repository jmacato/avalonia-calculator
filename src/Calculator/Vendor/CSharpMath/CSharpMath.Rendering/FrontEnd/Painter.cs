...
namespace CSharpMath.Rendering.FrontEnd {
  ...
  public abstract class Painter<TCanvas, TContent, TColor> : ICSharpMathAPI<TContent, TColor> where TContent : class {
    ...
    Fonts? _fonts;
    protected Fonts Fonts => _fonts ?? throw new InvalidOperationException(
      "Set LocalTypefaces to an Avalonia GlyphTypeface with an OpenType MATH table before rendering.");
    float __fontSize = DefaultFontSize;
    /// <summary>Unit of measure: points</summary>
    public float FontSize { get => __fontSize; set {
      __fontSize = value;
      if (_fonts != null) _fonts = new Fonts(_fonts, value);
      SetRedisplay();
    } }
    IReadOnlyList<GlyphTypeface> __localTypefaces = Array.Empty<GlyphTypeface>();
    public IEnumerable<GlyphTypeface> LocalTypefaces { get => __localTypefaces; set {
      __localTypefaces = value?.ToArray() ?? Array.Empty<GlyphTypeface>();
      _fonts = __localTypefaces.Count == 0 ? null : new Fonts(__localTypefaces, FontSize);
      SetRedisplay();
    } }
    ...
    protected void UpdateDisplay(float textPainterCanvasWidth) {
      ...
      if (Display == null && DisplayErrorInline && ErrorMessage != null) {
        ...
        for (var i = 0; i < errorLines.Length; i++) {
          ...
          y -= run.Descent
             + run.Run.Glyphs.Max(g => g.Typeface.Typeface.Metrics.LineGap * font.ScaleFor(g.Typeface));
          runs.Add(run);
        }
