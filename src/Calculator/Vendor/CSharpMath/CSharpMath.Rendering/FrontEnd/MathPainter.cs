...
namespace CSharpMath.Rendering.FrontEnd
{
    ...
    public abstract class MathPainter<TCanvas, TColor> : Painter<TCanvas, Atom.MathList, TColor>
    {
        ...
        public override IDisplay<MathFontSet, Glyph>? Display { get; protected set; }
        ...
        public override RectangleF Measure(float textPainterCanvasWidth = float.NaN) => base.Measure(textPainterCanvasWidth);
        protected override void SetRedisplay() => _displayChanged = true;
        protected override void UpdateDisplayCore(float textPainterCanvasWidth)
        {
            if (_displayChanged)
            {
                Display = Content == null
                  ? null
                  : Typesetter.CreateLine(Content, FontSet, TypesettingContext.Instance, LineStyle);
                _displayChanged = false;
            }
        }
        public override void Draw(TCanvas canvas, TextAlignment alignment = TextAlignment.None, Thickness padding = default, float offsetX = 0, float offsetY = 0)
        {
