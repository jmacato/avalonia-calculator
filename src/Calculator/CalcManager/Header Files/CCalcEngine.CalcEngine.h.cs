...
internal sealed partial class CCalcEngine
{
    ...
    {
        return s_engineStrings[ids.ToString(System.Globalization.CultureInfo.CurrentCulture)];
    }
    ...
    // static wstring_view OpCodeToUnaryString(int nOpCode, bool fInv, RatPakAngleType angletype);
    ...
    RatPakNumberFormat m_nFE; // Scientific notation conversion flag
    ...
    RatPakAngleType m_angletype; // Current Angle type when in dec mode. one of deg, rad or grad
    CCalcEngineNUM_WIDTH m_numwidth; // one of qword, dword, word or byte mode.
    ...
    //  void SetRadixTypeAndNumWidth(RadixType radixtype, CCalcEngineNUM_WIDTH numwidth);
    //  int32_t DwWordBitWidthFromNumWidth(CCalcEngineNUM_WIDTH numwidth);
