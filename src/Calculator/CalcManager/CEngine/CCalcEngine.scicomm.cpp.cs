...
internal sealed partial class CCalcEngine
{
    ...
    public void ProcessCommandWorker(OpCode wParam)
    {
        ...
        if (TryPrepareCommand(ref wParam))
        {
            return;
        }

        // Toggle Record/Display mode if appropriate.
        UpdateInputMode(wParam);
        // Interpret digit keys.
        if (TryHandleDigitCommand(wParam))
        {
            return;
        }

        // BINARY OPERATORS:
        if (TryHandleBinaryCommand(wParam))
        {
            return;
        }

        // UNARY OPERATORS:
        if (TryHandleUnaryCommand(wParam))
        {
            return;
        }

        // Tiny binary edit windows clicked. Toggle that bit and update display
        if (TryHandleBitEditCommand(wParam))
        {
            return;
        }

        /* Now branch off to do other commands and functions.                 */
        _ = HandleProcessCommandGroup1(wParam) || HandleProcessCommandGroup2(wParam) || HandleProcessCommandGroup3(wParam) || HandleProcessCommandGroup4(wParam) || HandleProcessCommandGroup5(wParam) || HandleProcessCommandGroup6(wParam);
    }

    private bool TryPrepareCommand(ref uint wParam)
    {
        // Save the last command.  Some commands are not saved in this manor, these
        // commands are:
        // Inv, Deg, Rad, Grad, Stat, FE, MClear, Back, and Exp.  The excluded
    ...
    private void UpdateInputMode(uint wParam)
    {
        // Toggle Record/Display mode if appropriate.
        if (m_bRecord)
        {
            if (IsBinOpCode(wParam) || IsUnaryOpCode(wParam) || IsOpInRange(wParam, CCommand.IDC_FE, CCommand.IDC_MMINUS) || IsOpInRange(wParam, CCommand.IDC_OPENP, CCommand.IDC_CLOSEP) || IsOpInRange(wParam, CCommand.IDM_HEX, CCommand.IDM_BIN) || IsOpInRange(wParam, CCommand.IDM_QWORD, CCommand.IDM_BYTE) || IsOpInRange(wParam, CCommand.IDM_DEG, CCommand.IDM_GRAD) || IsOpInRange(wParam, CCommand.IDC_BINEDITSTART, CCommand.IDC_BINEDITEND) || (CCommand.IDC_INV == wParam) || (CCommand.IDC_SIGN == wParam && 10 != m_radix) || (CCommand.IDC_RAND == wParam) || (CCommand.IDC_EULER == wParam))
            {
                m_bRecord = false;
    ...
    private bool TryHandleDigitCommand(uint wParam)
    {
        // Interpret digit keys.
    ...
    private bool TryHandleBinaryCommand(uint wParam)
    {
        // BINARY OPERATORS:
    ...
    private bool TryHandleUnaryCommand(uint wParam)
    {
        // UNARY OPERATORS:
        if (IsUnaryOpCode(wParam) || (wParam == CCommand.IDC_DEGREES))
        {
            ...
            if (IsTrigonometricCommand(wParam))
            {
            ...
            and have been used */
            if (m_bInv && ResetsInverseMode(wParam))
            {
                m_bInv = false;
            }
    ...
    private static bool IsTrigonometricCommand(uint command)
    {
        return command == CCommand.IDC_SIN || command == CCommand.IDC_COS || command == CCommand.IDC_TAN ||
            command == CCommand.IDC_SINH || command == CCommand.IDC_COSH || command == CCommand.IDC_TANH ||
            command == CCommand.IDC_SEC || command == CCommand.IDC_CSC || command == CCommand.IDC_COT ||
            command == CCommand.IDC_SECH || command == CCommand.IDC_CSCH || command == CCommand.IDC_COTH;
    }

    private static bool ResetsInverseMode(uint command)
    {
        return command == CCommand.IDC_CHOP || command == CCommand.IDC_SIN || command == CCommand.IDC_COS ||
            command == CCommand.IDC_TAN || command == CCommand.IDC_LN || command == CCommand.IDC_DMS ||
            command == CCommand.IDC_DEGREES || command == CCommand.IDC_SINH || command == CCommand.IDC_COSH ||
            command == CCommand.IDC_TANH || command == CCommand.IDC_SEC || command == CCommand.IDC_CSC ||
            command == CCommand.IDC_COT || command == CCommand.IDC_SECH || command == CCommand.IDC_CSCH ||
            command == CCommand.IDC_COTH;
    }

    private bool TryHandleBitEditCommand(uint wParam)
    {
        // Tiny binary edit windows clicked. Toggle that bit and update display
    ...
    private bool HandleProcessCommandGroup1(uint wParam)
    {
        switch (wParam)
        {
            ...
            case CCommand.IDC_EQU:
                while (m_openParenCount > 0)
                {
                    ...
                    m_nTempCom = (int)wParam; // put back in the state where last op seen was CCommand.IDC_CLOSEP, and current op is CCommand.IDC_EQU
                }
    ...
    private bool HandleProcessCommandGroup2(uint wParam)
    {
        switch (wParam)
        {
            ...
            case CCommand.IDC_CLOSEP:
                ...
                if (IsParenthesisCommandInvalid(wParam))
                {
                ...
                if (wParam == CCommand.IDC_OPENP)
                {
                    ...
                    if (IsDigitOpCode((OpCode)m_nLastCom) || IsUnaryOpCode((OpCode)m_nLastCom) || m_nLastCom == CCommand.IDC_PNT || m_nLastCom == CCommand.IDC_CLOSEP)
                    {
    ...
    private bool IsParenthesisCommandInvalid(uint command)
    {
        return (m_openParenCount >= MAXPRECDEPTH && command == CCommand.IDC_OPENP) ||
            (m_openParenCount == 0 && command != CCommand.IDC_OPENP) ||
            (m_precedenceOpCount >= MAXPRECDEPTH && m_nPrecOp[m_precedenceOpCount - 1] != 0);
    }

    private bool HandleProcessCommandGroup3(uint wParam)
    {
        switch (wParam)
        {
            ...
            case CCommand.IDM_BIN:
                {
                    ...
                    SetRadixTypeAndNumWidth((RadixType)(wParam - CCommand.IDM_HEX), default(CCalcEngineNUM_WIDTH) - 1);
            ...
            case CCommand.IDM_BYTE:
                ...
                // TODO: Double check this with original
                SetRadixTypeAndNumWidth(default(RadixType) - 1, (CCalcEngineNUM_WIDTH)(wParam - CCommand.IDM_QWORD));
                break;
            default:
                return false;
        }
    ...
    private bool HandleProcessCommandGroup4(uint wParam)
    {
        switch (wParam)
        {
            ...
            case CCommand.IDM_GRAD:
                m_angletype = (RatPakAngleType)(wParam - CCommand.IDM_DEG);
                break;
    ...
    private bool HandleProcessCommandGroup5(uint wParam)
    {
        switch (wParam)
        {
            case CCommand.IDC_STORE:
            case CCommand.IDC_MCLEAR:
                m_memoryValue = wParam == CCommand.IDC_STORE ? TruncateNumForIntMath(m_currentVal) : new Rational(m_ratPak, 0);
                break;
            ...
            case CCommand.IDC_RAND:
                if (!m_fIntegerMode)
                {
                    ...
                    WString str = GenerateRandomNumber().ToString($"F{m_precision}", System.Globalization.CultureInfo.CurrentCulture);
    ...
    private bool HandleProcessCommandGroup6(uint wParam)
    {
        switch (wParam)
        {
            case CCommand.IDC_FE:
                // Toggle exponential notation display.
                m_nFE = m_nFE == RatPakNumberFormat.Float ? RatPakNumberFormat.Scientific : RatPakNumberFormat.Float;
                DisplayNum();
                break;
    ...
    public void CheckAndAddLastBinOpToHistory(bool addToHistory = true)
    {
        ...
        else if (m_HistoryCollector.FOpndAddedToHistory() && !m_bError)
        {
            ...
            if ((IsUnaryOpCode((OpCode)m_nLastCom) || CCommand.IDC_SIGN == m_nLastCom || CCommand.IDC_CLOSEP == m_nLastCom) && 0 == m_openParenCount)
            {
                if (addToHistory)
                {
    ...
    private static readonly Dictionary<int32_t, CCalcEngineFunctionNameElement> operatorStringTable = new()
    {
        {
            CCommand.IDC_CHOP,
            new CCalcEngineFunctionNameElement
            {
                degreeString = "",
                inverseDegreeString = EngineStrings.SIDS_FRAC
            }
        },
        {
            CCommand.IDC_SIN,
            new CCalcEngineFunctionNameElement
            {
                degreeString = EngineStrings.SIDS_SIND,
                inverseDegreeString = EngineStrings.SIDS_ASIND,
                radString = EngineStrings.SIDS_SINR,
                inverseRadString = EngineStrings.SIDS_ASINR,
                gradString = EngineStrings.SIDS_SING,
                inverseGradString = EngineStrings.SIDS_ASING
            }
        },
        {
            CCommand.IDC_COS,
            new CCalcEngineFunctionNameElement
            {
                degreeString = EngineStrings.SIDS_COSD,
                inverseDegreeString = EngineStrings.SIDS_ACOSD,
                radString = EngineStrings.SIDS_COSR,
                inverseRadString = EngineStrings.SIDS_ACOSR,
                gradString = EngineStrings.SIDS_COSG,
                inverseGradString = EngineStrings.SIDS_ACOSG
            }
        },
        {
            CCommand.IDC_TAN,
            new CCalcEngineFunctionNameElement
            {
                degreeString = EngineStrings.SIDS_TAND,
                inverseDegreeString = EngineStrings.SIDS_ATAND,
                radString = EngineStrings.SIDS_TANR,
                inverseRadString = EngineStrings.SIDS_ATANR,
                gradString = EngineStrings.SIDS_TANG,
                inverseGradString = EngineStrings.SIDS_ATANG
            }
        },
        {
            CCommand.IDC_SINH,
            ...
            {
                degreeString = "",
                inverseDegreeString = EngineStrings.SIDS_ASINH
            }
        },
        {
            CCommand.IDC_COSH,
            ...
            {
                degreeString = "",
                inverseDegreeString = EngineStrings.SIDS_ACOSH
            }
        },
        {
            CCommand.IDC_TANH,
            ...
            {
                degreeString = "",
                inverseDegreeString = EngineStrings.SIDS_ATANH
            }
        },
        {
            CCommand.IDC_SEC,
            new CCalcEngineFunctionNameElement
            {
                degreeString = EngineStrings.SIDS_SECD,
                inverseDegreeString = EngineStrings.SIDS_ASECD,
                radString = EngineStrings.SIDS_SECR,
                inverseRadString = EngineStrings.SIDS_ASECR,
                gradString = EngineStrings.SIDS_SECG,
                inverseGradString = EngineStrings.SIDS_ASECG
            }
        },
        {
            CCommand.IDC_CSC,
            new CCalcEngineFunctionNameElement
            {
                degreeString = EngineStrings.SIDS_CSCD,
                inverseDegreeString = EngineStrings.SIDS_ACSCD,
                radString = EngineStrings.SIDS_CSCR,
                inverseRadString = EngineStrings.SIDS_ACSCR,
                gradString = EngineStrings.SIDS_CSCG,
                inverseGradString = EngineStrings.SIDS_ACSCG
            }
        },
        {
            CCommand.IDC_COT,
            new CCalcEngineFunctionNameElement
            {
                degreeString = EngineStrings.SIDS_COTD,
                inverseDegreeString = EngineStrings.SIDS_ACOTD,
                radString = EngineStrings.SIDS_COTR,
                inverseRadString = EngineStrings.SIDS_ACOTR,
                gradString = EngineStrings.SIDS_COTG,
                inverseGradString = EngineStrings.SIDS_ACOTG
            }
        },
        {
            CCommand.IDC_SECH,
            new CCalcEngineFunctionNameElement
            {
                degreeString = EngineStrings.SIDS_SECH,
                inverseDegreeString = EngineStrings.SIDS_ASECH
            }
        },
        {
            CCommand.IDC_CSCH,
            new CCalcEngineFunctionNameElement
            {
                degreeString = EngineStrings.SIDS_CSCH,
                inverseDegreeString = EngineStrings.SIDS_ACSCH
            }
        },
        {
            CCommand.IDC_COTH,
            new CCalcEngineFunctionNameElement
            {
                degreeString = EngineStrings.SIDS_COTH,
                inverseDegreeString = EngineStrings.SIDS_ACOTH
            }
        },
        {
            CCommand.IDC_LN,
            ...
            {
                degreeString = "",
                inverseDegreeString = EngineStrings.SIDS_POWE
            }
        },
        {
            CCommand.IDC_SQR,
            new CCalcEngineFunctionNameElement
            {
                degreeString = EngineStrings.SIDS_SQR
            }
        },
        {
            CCommand.IDC_CUB,
            new CCalcEngineFunctionNameElement
            {
                degreeString = EngineStrings.SIDS_CUBE
            }
        },
        {
            CCommand.IDC_FAC,
            new CCalcEngineFunctionNameElement
            {
                degreeString = EngineStrings.SIDS_FACT
            }
        },
        {
            CCommand.IDC_REC,
            new CCalcEngineFunctionNameElement
            {
                degreeString = EngineStrings.SIDS_RECIPROC
            }
        },
        {
            CCommand.IDC_DMS,
            ...
            {
                degreeString = "",
                inverseDegreeString = EngineStrings.SIDS_DEGREES
            }
        },
        {
            CCommand.IDC_SIGN,
            new CCalcEngineFunctionNameElement
            {
                degreeString = EngineStrings.SIDS_NEGATE
            }
        },
        {
            CCommand.IDC_DEGREES,
            new CCalcEngineFunctionNameElement
            {
                degreeString = EngineStrings.SIDS_DEGREES
            }
        },
        {
            CCommand.IDC_POW2,
            new CCalcEngineFunctionNameElement
            {
                degreeString = EngineStrings.SIDS_TWOPOWX
            }
        },
        {
            CCommand.IDC_LOGBASEY,
            new CCalcEngineFunctionNameElement
            {
                degreeString = EngineStrings.SIDS_LOGBASEY
            }
        },
        {
            CCommand.IDC_ABS,
            new CCalcEngineFunctionNameElement
            {
                degreeString = EngineStrings.SIDS_ABS
            }
        },
        {
            CCommand.IDC_CEIL,
            new CCalcEngineFunctionNameElement
            {
                degreeString = EngineStrings.SIDS_CEIL
            }
        },
        {
            CCommand.IDC_FLOOR,
            new CCalcEngineFunctionNameElement
            {
                degreeString = EngineStrings.SIDS_FLOOR
            }
        },
        {
            CCommand.IDC_NAND,
            new CCalcEngineFunctionNameElement
            {
                degreeString = EngineStrings.SIDS_NAND
            }
        },
        {
            CCommand.IDC_NOR,
            new CCalcEngineFunctionNameElement
            {
                degreeString = EngineStrings.SIDS_NOR
            }
        },
        {
            CCommand.IDC_RSHFL,
            new CCalcEngineFunctionNameElement
            {
                degreeString = EngineStrings.SIDS_RSH
            }
        },
        {
            CCommand.IDC_RORC,
            new CCalcEngineFunctionNameElement
            {
                degreeString = EngineStrings.SIDS_ROR
            }
        },
        {
            CCommand.IDC_ROLC,
            new CCalcEngineFunctionNameElement
            {
                degreeString = EngineStrings.SIDS_ROL
            }
        },
        {
            CCommand.IDC_CUBEROOT,
            new CCalcEngineFunctionNameElement
            {
                degreeString = EngineStrings.SIDS_CUBEROOT
            }
        },
        {
            CCommand.IDC_MOD,
            new CCalcEngineFunctionNameElement
            {
                degreeString = EngineStrings.SIDS_MOD,
                programmerModeString = EngineStrings.SIDS_PROGRAMMER_MOD
            }
        },
    };
    public wstring_view OpCodeToUnaryString(int nOpCode, bool fInv, RatPakAngleType angletype)
    {
        // Try to lookup the ID in the UFNE table
        WString ids = "";
        if (operatorStringTable.TryGetValue(nOpCode, out var element)) //(var pair = operatorStringTable.find(nOpCode); pair != operatorStringTable.end())
        {
            if (!element.hasAngleStrings || RatPakAngleType.Degrees == angletype)
            {
                if (fInv)
                {
            ...
            else if (RatPakAngleType.Radians == angletype)
            {
                if (fInv)
                {
            ...
            else if (RatPakAngleType.Gradians == angletype)
            {
                if (fInv)
                {
    ...
    public WString GetStringForDisplay(Rational rat, uint32_t radix)
    {
        ...
        else
        {
            ...
            catch (Exception exception) when (exception is CalcErrException or ArithmeticException or ArgumentException or InvalidOperationException)
            {
            }
        }
    ...
    {
        return System.Security.Cryptography.RandomNumberGenerator.GetInt32(int.MaxValue) / (double)int.MaxValue;
    }
}
