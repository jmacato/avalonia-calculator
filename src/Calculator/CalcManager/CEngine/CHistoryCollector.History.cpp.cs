...
internal sealed partial class CHistoryCollector
{
    ...
    static void Truncate<T>(List<T> v, int index)
    {
        if (index >= v.Count)
        {
            throw new InvalidOperationException("E_BOUNDS");
        }
    ...
    public CHistoryCollector(CCalcEngine cCalcEngine, ICalcDisplay pCalcDisplay, IHistoryDisplay? pHistoryDisplay, wchar_t decimalSymbol)
    {
    ...
    //
    public void AddUnaryOpToHistory(int nOpCode, bool fInv, RatPakAngleType angletype)
    {
        int iCommandEnd;
        ...
        {
            IOperatorCommand spExpressionCommand = new CUnaryCommand(nOpCode);
            ...
            else
            {
                ...
                if (angletype == RatPakAngleType.Degrees)
                {
                    angleOpCode = CalculationManager.Command.DEG;
                }
                else if (angletype == RatPakAngleType.Radians)
                {
                    angleOpCode = CalculationManager.Command.RAD;
                }
                else // (angletype == RatPakAngleType.Gradians)
                {
                    angleOpCode = CalculationManager.Command.GRAD;
                }

                int command = nOpCode;
                _ = HandleUnaryHistoryGroup1(nOpCode, ref command, fInv, ref spExpressionCommand, angleOpCode) || HandleUnaryHistoryGroup2(nOpCode, ref command, fInv, ref spExpressionCommand, angleOpCode);
            }

            iCommandEnd = AddCommand(spExpressionCommand);
            WString operandStr = m_cCalcEngine.OpCodeToUnaryString(nOpCode, fInv, angletype);
            ;
    ...
    private static bool HandleUnaryHistoryGroup1(int nOpCode, ref int command, bool fInv, ref global::IOperatorCommand spExpressionCommand, global::CalculationManager.Command angleOpCode)
    {
        switch (nOpCode)
        {
            case CCommand.IDC_SIN:
                command = fInv ? (int)(CalculationManager.Command.ASIN) : CCommand.IDC_SIN;
                spExpressionCommand = new CUnaryCommand((int)(angleOpCode), command);
                break;
            case CCommand.IDC_COS:
                command = fInv ? (int)(CalculationManager.Command.ACOS) : CCommand.IDC_COS;
                spExpressionCommand = new CUnaryCommand((int)(angleOpCode), command);
                break;
            case CCommand.IDC_TAN:
                command = fInv ? (int)(CalculationManager.Command.ATAN) : CCommand.IDC_TAN;
                spExpressionCommand = new CUnaryCommand((int)(angleOpCode), command);
                break;
            case CCommand.IDC_SINH:
                command = fInv ? (int)(CalculationManager.Command.ASINH) : CCommand.IDC_SINH;
                spExpressionCommand = new CUnaryCommand(command);
                break;
            case CCommand.IDC_COSH:
                command = fInv ? (int)(CalculationManager.Command.ACOSH) : CCommand.IDC_COSH;
                spExpressionCommand = new CUnaryCommand(command);
                break;
            case CCommand.IDC_TANH:
                command = fInv ? (int)(CalculationManager.Command.ATANH) : CCommand.IDC_TANH;
                spExpressionCommand = new CUnaryCommand(command);
                break;
            case CCommand.IDC_SEC:
                command = fInv ? (int)(CalculationManager.Command.ASEC) : CCommand.IDC_SEC;
                spExpressionCommand = new CUnaryCommand((int)(angleOpCode), command);
                break;
            default:
                return false;
    ...
    private static bool HandleUnaryHistoryGroup2(int nOpCode, ref int command, bool fInv, ref global::IOperatorCommand spExpressionCommand, global::CalculationManager.Command angleOpCode)
    {
        switch (nOpCode)
        {
            case CCommand.IDC_CSC:
                command = fInv ? (int)(CalculationManager.Command.ACSC) : CCommand.IDC_CSC;
                spExpressionCommand = new CUnaryCommand((int)(angleOpCode), command);
                break;
            case CCommand.IDC_COT:
                command = fInv ? (int)(CalculationManager.Command.ACOT) : CCommand.IDC_COT;
                spExpressionCommand = new CUnaryCommand((int)(angleOpCode), command);
                break;
            case CCommand.IDC_SECH:
                command = fInv ? (int)(CalculationManager.Command.ASECH) : CCommand.IDC_SECH;
                spExpressionCommand = new CUnaryCommand(command);
                break;
            case CCommand.IDC_CSCH:
                command = fInv ? (int)(CalculationManager.Command.ACSCH) : CCommand.IDC_CSCH;
                spExpressionCommand = new CUnaryCommand(command);
                break;
            case CCommand.IDC_COTH:
                command = fInv ? (int)(CalculationManager.Command.ACOTH) : CCommand.IDC_COTH;
                spExpressionCommand = new CUnaryCommand(command);
                break;
            case CCommand.IDC_LN:
                command = fInv ? (int)(CalculationManager.Command.POWE) : CCommand.IDC_LN;
                spExpressionCommand = new CUnaryCommand(command);
                break;
            default:
                spExpressionCommand = new CUnaryCommand(nOpCode);
    ...
    {
        if (m_spTokens is null || m_spCommands is null)
        {
            return;
        }
        ...
        // for (var token in m_spTokens)
        {
            ...
            if (commandPosition != -1)
            {
                ...
                if (expCommand != null && CalculationManager.CommandType.OperandCommand == expCommand.GetCommandType())
                {
