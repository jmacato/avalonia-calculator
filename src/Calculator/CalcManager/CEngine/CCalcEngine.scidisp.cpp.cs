...
internal sealed partial class CCalcEngine
{
    ...
    *   m_currentVal, m_numberString
    ...
    //
    private static CCalcEngineLASTDISP gldPrevious = new CCalcEngineLASTDISP
    {
        value = null,
        precision = -1,
        radix = 0,
        nFE = -1,
        numwidth = (CCalcEngineNUM_WIDTH)(-1),
        fIntMath = false,
        bRecord = false,
        bUseSep = false
    };
    ...
    void DisplayNum()
    {
        ...
        //  called.
        //
        if (m_bRecord || gldPrevious.value != m_currentVal || gldPrevious.precision != m_precision || gldPrevious.radix != m_radix || gldPrevious.nFE != (int)m_nFE || !gldPrevious.bUseSep || gldPrevious.numwidth != m_numwidth || gldPrevious.fIntMath != m_fIntegerMode || gldPrevious.bRecord != m_bRecord)
        {
    ...
    *
    \****************************************************************************/
    public static List<uint32_t> DigitGroupingStringToGroupingVector(wstring_view groupingString)
    {
   ...
   *
   \***************************************************************************/
    public string GroupDigits(string delimiter, List<uint> grouping, string displayString, bool isNumNegative = false)
    {
        ...
        var exp = displayString.IndexOf('e', StringComparison.Ordinal);
        var hasExponent = (exp != -1);
        // Find the position of decimal point in the string
        var dec = displayString.IndexOf(m_decimalSeparator, StringComparison.Ordinal);
