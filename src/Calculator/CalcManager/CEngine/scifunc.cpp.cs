...
Rational SciCalcFunctions(Rational rat, uint32_t op)
{
    ...
    try
    {
        ...
        case CCommand.IDC_CUB: /* Cubing and cube root functions. */
            result = CCommand.IDC_CUBEROOT == op ? RationalMath.Root(m_ratPak, rat, new Rational(m_ratPak, 3))
        ...
        case CCommand.IDC_CEIL:
            result = (RationalMath.Frac(m_ratPak, rat) > new Rational(m_ratPak, 0))
                ? RationalMath.Integer(m_ratPak, rat + new Rational(m_ratPak, 1)) : RationalMath.Integer(m_ratPak, rat);
            break;
...
{
    var actualId = (EngineStrings.IDS_ERRORS_FIRST + m_ratPak.SCODE_CODE(nError));

    wstring errorString = GetString(actualId.ToString());
