...
internal sealed partial class RatPak
{
    ...
    {
        boolrat(ref pa, b, (int)RatPakBOOL_FUNCS.FUNC_AND, radix, precision);
    }

    public void orrat(ref PRAT pa, PRAT b, uint32_t radix, int32_t precision)
    {
        boolrat(ref pa, b, (int)RatPakBOOL_FUNCS.FUNC_OR, radix, precision);
    }

    public void xorrat(ref PRAT pa, PRAT b, uint32_t radix, int32_t precision)
    {
        boolrat(ref pa, b, (int)RatPakBOOL_FUNCS.FUNC_XOR, radix, precision);
    }
    ...
    void boolnum(ref PNUMBER pa, PNUMBER b, int func)
    {
        ...
        for (; cdigits > 0; cdigits--, mexp++)
        {
            ...
            switch (func)
            {
                case (int)RatPakBOOL_FUNCS.FUNC_AND:
                    pchc[pchcCnt++] = da & db;
                    break;
                case (int)RatPakBOOL_FUNCS.FUNC_OR:
                    pchc[pchcCnt++] = da | db;
                    break;
                case (int)RatPakBOOL_FUNCS.FUNC_XOR:
                    pchc[pchcCnt++] = da ^ db;
                    break;
            }
        }
