...
internal sealed partial class RatPak
{
    ...
    public const int32_t CALC_INTSAFE_E_ARITHMETIC_OVERFLOW = unchecked((int32_t)0x80070216U); // 0x216 = 534 = ERROR_ARITHMETIC_OVERFLOW
    ...
    //
    //-----------------------------------------------------------------------------
    private static void _dupnum([NotNull] ref PNUMBER? dest, RatPakNUMBER src)
    {
        dest = new PNUMBER
        {
            cdigit = src.cdigit,
        ...
        // memcpy(dest, src, (int)(sizeof(RatPakNUMBER) + ((src).cdigit) * (sizeof(MANTTYPE))));
    }
    ...
    private static PNUMBER _createnum(uint32_t size)
    {
        ...
        if (SUCCEEDED(Calc_ULongAdd(size, 1, out cbAlloc)) && SUCCEEDED(Calc_ULongMult(cbAlloc, (uint32_t)sizeof(MANTTYPE), out cbAlloc)) && SUCCEEDED(Calc_ULongAdd(cbAlloc, RatPakNUMBER.ConstSizeOf, out cbAlloc)))
        {
    ...
    //
    //-----------------------------------------------------------------------------
    public PRAT? StringToRat(bool mantissaIsNegative, wstring_view mantissa, bool exponentIsNegative, wstring_view exponent, uint32_t radix, int32_t precision)
    {
    ...
    static readonly uint8_t[][] machine = [ //        DP,     ZR,      NZ,      SG,     EX
    // START
 ...
 [ERR, ERR, ERR, ERR, ERR]];
    static wchar_t NormalizeCharDigit(wchar_t c, uint32_t radix)
    {
        ...
        if (radix >= DIGITS.IndexOf('A', StringComparison.Ordinal) && radix <= DIGITS.IndexOf('Z', StringComparison.Ordinal))
        {
    ...
    public PNUMBER? StringToNumber(wstring_view numberString, uint32_t radix, int32_t precision)
    {
        ...
        foreach (var c in numberString)
        {
            ...
            _ = HandleNumberCharacterGroup1(curChar, ref state) || HandleNumberCharacterGroup2(curChar, radix, ref state);
                ...
                case EXPD:
                    {
                        ...
                        var pos = DIGITS.IndexOf(curChar, StringComparison.Ordinal);
    ...
    private static bool HandleNumberCharacterGroup1(char curChar, ref byte state)
    {
        switch (curChar)
        {
            case '-':
    ...
    private static bool HandleNumberCharacterGroup2(char curChar, uint radix, ref byte state)
    {
        switch (curChar)
        {
            case '^':
    ...
    //          fmt, one of RatPakNumberFormat.Float, RatPakNumberFormat.Scientific or
    //          RatPakNumberFormat.Engineering
    ...
    //
    //-----------------------------------------------------------------------------
    public WString NumberToString(ref PNUMBER pnum, RatPakNumberFormat format, uint32_t radix, int32_t precision)
    {
        ...
        RatPakNumberFormat oldFormat = format;
        if (exponent > precision && format == RatPakNumberFormat.Float)
        {
            // Force scientific mode to prevent user from assuming 33rd digit is exact.
            format = RatPakNumberFormat.Scientific;
        }
        ...
        PNUMBER? round = null;
        PrepareNumberRounding(pnum, precision, ref length, exponent, ref round, radix, ref format);
        if (round != null)
        {
        ...
        // RatPakNumberFormat.Scientific or RatPakNumberFormat.Engineering was specified.
        PrepareScientificFormat(format, ref useSciForm, ref eout, ref exponent);
    ...
    private void PrepareNumberRounding(global::CalcEngine.RatPakNUMBER pnum, int precision, ref int length, int exponent, ref global::CalcEngine.RatPakNUMBER? round, uint radix, ref global::CalcEngine.RatPakNumberFormat format)
    {
        if (!zernum(pnum) && (pnum.cdigit >= precision || (length - exponent > precision && exponent >= -MAX_ZEROS_AFTER_DECIMAL)))
        {
            ...
            if (exponent > 0 || format == RatPakNumberFormat.Float)
            {
        ...
        if (format == RatPakNumberFormat.Float)
        {
            ...
            {
                if (exponent >= -MAX_ZEROS_AFTER_DECIMAL && round is not null)
                {
                    round.exp -= exponent;
                    length = precision + exponent;
                }
    ...
    private static void PrepareScientificFormat(global::CalcEngine.RatPakNumberFormat format, ref bool useSciForm, ref int eout, ref int exponent)
    {
        // Case where too many digits are to the left of the decimal or
        // RatPakNumberFormat.Scientific or RatPakNumberFormat.Engineering was specified.
        if ((format == RatPakNumberFormat.Scientific) || (format == RatPakNumberFormat.Engineering))
        {
            useSciForm = true;
            if (eout != 0)
            {
                if (format == RatPakNumberFormat.Engineering)
                {
                    exponent = (eout % 3);
                    eout -= exponent;
                    exponent++;
    ...
    //              fmt, one of RatPakNumberFormat.Float, RatPakNumberFormat.Scientific, or RatPakNumberFormat.Engineering
    //              precision uint32_t
    //
    //  RETURN: string
    //
    ...
    //
    //-----------------------------------------------------------------------------
    public WString RatToString(ref PRAT prat, RatPakNumberFormat format, uint32_t radix, int32_t precision)
    {
