...
internal sealed class AutomationRequestRouter
{
    private readonly AutomationServer _server;
    private readonly string _token;
    private readonly int _port;

    public AutomationRequestRouter(AutomationServer server, string token, int port)
    {
        _server = server;
        _token = token;
        _port = port;
    ...
    private async Task RouteAsync(HttpListenerContext context)
    {
        ...
        if (!string.Equals(request.Headers["X-Calculator-Automation-Token"], _token, StringComparison.Ordinal))
        {
    ...
    private async Task<bool> RouteGetAsync(HttpListenerContext context, string path)
    {
        ...
        {
            case "/health":
                await WriteJsonAsync(context.Response, new AutomationHealthResponse("ok", Environment.ProcessId, _port), AutomationJsonContext.Default.AutomationHealthResponse).ConfigureAwait(false);
                return true;
            case "/tree":
                AutomationTreeResponse response = await Dispatcher.UIThread.InvokeAsync(_server.BuildTree);
    ...
    private async Task<bool> RoutePostAsync(HttpListenerContext context, string path)
    {
        switch (path)
        {
            case "/events/pointer":
                ...
                await Dispatcher.UIThread.InvokeAsync(() => _server.SendPointer(pointer));
                break;
            case "/events/click":
                ...
                await Dispatcher.UIThread.InvokeAsync(() => _server.ClickTarget(target.Target));
                break;
            case "/events/key":
                ...
                await Dispatcher.UIThread.InvokeAsync(() => _server.SendKey(key));
                break;
            case "/events/text":
                ...
                await Dispatcher.UIThread.InvokeAsync(() => _server.SendText(text.Text));
                break;
            case "/window/size":
                ...
                await Dispatcher.UIThread.InvokeAsync(() => _server.ResizeWindow(size));
    ...
    private static Task WriteOkAsync(HttpListenerResponse response) =>
        WriteJsonAsync(response, new AutomationActionResponse("ok"), AutomationJsonContext.Default.AutomationActionResponse);
