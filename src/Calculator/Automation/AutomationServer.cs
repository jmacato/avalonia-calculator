...
internal sealed class AutomationServer : IAsyncDisposable
{
    ...
    private IEnumerable<Control> EnumerateOpenPopupChildren() => new[]
    {
        _window
    }.Concat(_window.GetVisualDescendants().OfType<Control>()).OfType<Popup>().Where(popup => popup.IsOpen).Select(popup => popup.Child).OfType<Control>();
    ...
    private static bool IsEffectivelyVisible(Control control) => control.IsVisible && control.GetVisualAncestors().OfType<Control>().All(ancestor => ancestor.IsVisible);
    ...
    private static AutomationServerPointerButtonInfo ParsePointerButton(string? value) => value?.ToUpperInvariant() switch
    {
        null or "" or "LEFT" => new AutomationServerPointerButtonInfo(RawInputModifiers.LeftMouseButton, PointerUpdateKind.LeftButtonPressed, PointerUpdateKind.LeftButtonReleased, MouseButton.Left),
        "RIGHT" => new AutomationServerPointerButtonInfo(RawInputModifiers.RightMouseButton, PointerUpdateKind.RightButtonPressed, PointerUpdateKind.RightButtonReleased, MouseButton.Right),
        "MIDDLE" => new AutomationServerPointerButtonInfo(RawInputModifiers.MiddleMouseButton, PointerUpdateKind.MiddleButtonPressed, PointerUpdateKind.MiddleButtonReleased, MouseButton.Middle),
        _ => throw new InvalidDataException("Pointer button must be left, right, or middle.")
    };
    private IPointer GetPointer(string? value) => value?.ToUpperInvariant() switch
    {
