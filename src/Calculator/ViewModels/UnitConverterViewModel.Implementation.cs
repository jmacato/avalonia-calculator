...
public sealed partial class UnitConverterViewModel
{
    ...
    internal static UnitConverterViewModel FromPreparedModel(
        ...
        ISettingsStore settingsStore)
    {
        return new UnitConverterViewModel(model, settingsStore, initializeModel: false);
    }
    ...
    {
        return Units.FirstOrDefault(unit => unit.Id == target.Id) ?? UCM.Unit.EmptyUnit;
    }
    ...
    private static UCM.Command CommandFromButtonId(CalculatorButtonId button)
    {
    ...
    {
        ArgumentNullException.ThrowIfNull(stringToPaste);
