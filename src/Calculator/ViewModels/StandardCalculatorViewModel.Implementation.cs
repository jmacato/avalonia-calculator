...
public sealed partial class StandardCalculatorViewModel
{
        ...
        foreach (var command in expressionCommands)
        {
            ...
            if (commandType == CommandType.OperandCommand)
            {
                ...
                foreach (int nOCode in opndCommands)
                {
                    ...
                    {
                        commands.Add((int)Command.CommandSIGN);
                        fNeedIDCSign = false;
                    }
                }
            }
    ...
    {
        ArgumentNullException.ThrowIfNull(memorizedNumbers);
        LocalizationSettings localizer = LocalizationSettings.Instance;
    ...
    {
        return string.Create(source.Length - 1, (Source: source, Index: index), static (destination, state) =>
        {
    ...
    {
        return string.Create(source.Length + 1, (Source: source, Index: index, Character: character),
    ...
    {
        return operation is CalculatorButtonId.IsStandardMode
    ...
    {
        return operation is CalculatorButtonId.Degree
    ...
    {
        return !IsModeCommand(operation) && operation != CalculatorButtonId.FToE && !IsAngleCommand(operation);
    }

    private static bool ClearsFToEState(CalculatorButtonId operation)
    {
        return operation is CalculatorButtonId.Clear
    ...
    {
        return command is >= Command.Command0 and <= Command.Command9
    ...
    static Command ConvertToOperatorsEnum(CalculatorButtonId operation)
    {
    ...
    private static CalculatorButtonId MapFixedCharacter(char character)
    {
    ...
    {
        if (MemorizedNumbers != null && MemorizedNumbers.Count > 0 && memoryItemPosition is int boxedPosition and >= 0)
        {
    ...
    void Recalculate(bool fromHistory = false)
    {
        ...
        List<IExpressionCommand> savedCommands = [.. m_commands];
        List<int> currentCommands = GetCommandsFromExpressionCommands(m_commands);
        List<(string, int)> savedTokens = [.. m_tokens];
        ...
        {
            m_standardCalculatorManager.SendCommand((Command)command);
        }
    ...
    static bool IsRecoverableCommand(Command command)
    {
        ...
        if (Command.CommandBINEDITSTART <= command && command <= Command.BinEditEnd)
        {
            return true;
        }
    ...
    void ValueBitLengthSet(BitLength value)
    {
    ...
    public StandardCalculatorSnapshot Snapshot
    {
        get
        {
            var result = new StandardCalculatorSnapshot();
            result.CalcManager = new CalcManagerSnapshot(m_standardCalculatorManager);
            result.PrimaryDisplay = new PrimaryDisplaySnapshot(m_DisplayValue, m_IsInError);
            if (m_tokens.Count != 0 && m_commands.Count != 0)
            {
                result.ExpressionDisplay = new ExpressionDisplaySnapshot(m_tokens, m_commands);
            }
