...
public abstract class SettingsStoreBase : ISettingsStore
{
    ...
    public void Update(Func<AppSettings, AppSettings> update)
    {
        ...
        lock (_sync)
        {
            next = update(_current).Normalize();
            if (next == _current)
            {
                return;
            }
...
{
    public InMemorySettingsStore(AppSettings? current = null)
        : base(current ?? new AppSettings())
    {
    }
...
public sealed class JsonSettingsStore : SettingsStoreBase
{
    ...
    private JsonSettingsStore(string path, AppSettings current)
        : base(current)
    {
        _path = path;
    }
