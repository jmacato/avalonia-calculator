...
internal sealed class AdaptiveCurveSamplerSamplingContext : IDisposable
{
    ...
    public AdaptiveCurveSamplerSamplingContext(CurveEvaluator evaluator, SamplingViewport viewport, SamplingOptions options, CancellationToken cancellationToken)
    {
        _evaluator = evaluator;
        Viewport = viewport;
        Options = options;
        _cancellationToken = cancellationToken;
        _pointBuffer = ArrayPool<GraphPoint>.Shared.Rent(options.MaximumVertices);
    }
    ...
    public bool ShouldStop => _segmentBudgetExceeded || _evaluationBudgetExceeded || _cancellationToken.IsCancellationRequested;
    public bool CannotContinue => _evaluationBudgetExceeded || _cancellationToken.IsCancellationRequested;
    ...
    {
        Break();
        return new SampledCurve(_components.ToImmutableArray(), _evaluationCount, _vertexCount, HasMissingData || _cancellationToken.IsCancellationRequested, BudgetExceeded);
    }
