...
public readonly record struct SamplingViewport(AxisRange XRange, AxisRange YRange, double Width, double Height)
{
    ...
    public GraphPoint ToScreen(double x, double y)
    {
        return new GraphPoint((x - XRange.Minimum) / XRange.Length * Width, (YRange.Maximum - y) / YRange.Length * Height);
    }

    public GraphPoint ToUser(double screenX, double screenY)
    {
        return new GraphPoint(XRange.Minimum + screenX / Width * XRange.Length, YRange.Maximum - screenY / Height * YRange.Length);
    }
