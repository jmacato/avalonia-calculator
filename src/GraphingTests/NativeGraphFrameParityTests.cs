...
public sealed class NativeGraphFrameParityTests
{
    ...
    {
        RenderedGraph rendered = Render("x");
        StrokePathCommand[] grid = rendered.Frame.Commands
            .OfType<StrokePathCommand>()
            .Where(command => SameRgb(command.Paint.Color, NativeGrid))
            .ToArray();
    ...
    public void AxesLabelsAndClearingRectanglesMatchCapturedDrawOrder()
    {
        ...
        StrokePathCommand[] axes = commands
            .OfType<StrokePathCommand>()
            .Where(command => command.Paint.Color == new Color(0, 0, 0))
            .ToArray();
        ...
        Assert.Equal(
            new[] { "x", "-6", "-4", "-2", "0", "2", "4", "6", "y", "-10", "-5", "5", "10" },
            glyphs.Select(glyph => glyph.Text));
    ...
    public void DashedDiagonalUsesCapturedEuclideanCadence()
    {
        var frame = new GraphFrame(
            ...
            [new StrokePathCommand(
                new GraphPath([new GraphPoint(0, 90), new GraphPoint(90, 0)]),
                new GraphPaint(NativeBlue, 3, LineStyle.Dash))]);

        using SKBitmap bitmap = SKBitmap.Decode(SkiaGraphFrameRasterizer.EncodePng(frame).ToArray())
                                ?? throw new InvalidOperationException("Skia did not decode its PNG output.");
    ...
    public void MarchingSquaresStitchesBothEndsIntoOneConnectedBoundary()
    {
        var viewport = new SamplingViewport(
            ...
            new AxisRange(-11.1236797274276, 11.1236797274276),
            NativeWidth,
            NativeHeight);

        InequalityMesh mesh = MarchingSquares.Build(
            (x, y) => y - x,
            value => value > 0,
            viewport,
            columns: 73,
        ...
        double yMax = 11.1236797274276)
    {
        ...
        IReadOnlyList<IEquation> equations = graph.TryInitialize(expression)
            ?? throw new InvalidOperationException($"Initialize failed: {graph.GetInitializationError()}");
    ...
    private static StrokePathCommand[] BlueStrokes(GraphFrame frame) => frame.Commands
        .OfType<StrokePathCommand>()
        .Where(IsBlueStroke)
        .ToArray();
    ...
    private static bool IsVertical(StrokePathCommand command) =>
        command.Path.Points.Length == 2 &&
        Math.Abs(command.Path.Points[0].X - command.Path.Points[1].X) < 1e-10;

    private static bool IsHorizontal(StrokePathCommand command) =>
        command.Path.Points.Length == 2 &&
        Math.Abs(command.Path.Points[0].Y - command.Path.Points[1].Y) < 1e-10;

    private static double LineCoordinate(StrokePathCommand command) =>
        IsVertical(command) ? command.Path.Points[0].X : command.Path.Points[0].Y;
    ...
    private static List<(int Start, int Length)> BlueRuns(
        SKBitmap bitmap,
        double startX,
        double startY,
        double endX,
    ...
    private sealed record RenderedGraph(
        IGraph Graph,
        IEquation Equation,
