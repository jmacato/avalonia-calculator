...
public partial class RatPak
{
    ...
    public void gcdrat(ref PRAT pa, int32_t precision)
    {
        ...
        if (!zernum(pgcd))
        {
            divnumx(ref (a.pp), pgcd, precision);
            divnumx(ref (a.pq), pgcd, precision);
        }
    ...
    public void mulrat(ref PRAT pa, PRAT b, int32_t precision)
    {
        ...
        if (!zernum((pa).pp))
        {
            mulnumx(ref ((pa).pp), b.pp);
            mulnumx(ref ((pa).pq), b.pq);
            trimit(ref pa, precision);
        }
        else
        {
            // If it is zero, blast a one in the denominator.
            dupnum(ref ((pa).pq), num_one);
        }
    ...
    public void divrat(ref PRAT pa, PRAT b, int32_t precision)
    {
        if (!zernum((pa).pp))
        {
            ...
            mulnumx(ref ((pa).pp), b.pq);
            mulnumx(ref ((pa).pq), b.pp);

            if (zernum((pa).pq))
            {
                // raise an exception if the bottom is 0.
                throw new CalcErrException(CalcErr.CALC_E_DIVIDEBYZERO);
            }
    ...
    public void addrat(ref PRAT pa, PRAT b, int32_t precision)
    {
        ...
        if (equnum((pa).pq, b.pq))
        {
            ...
            b.pp.sign *= b.pq.sign;
            b.pq.sign = 1;
            addnum(ref ((pa).pp), b.pp, BASEX);
        }
        else
        {
            // Usual case q's aren't the same.
            dupnum(ref bot, (pa).pq);
            mulnumx(ref bot, b.pq);
            mulnumx(ref ((pa).pp), b.pq);
            mulnumx(ref ((pa).pq), b.pp);
            addnum(ref ((pa).pp), (pa).pq, BASEX);
            destroynum(ref (pa).pq);
            (pa).pq = bot;
            trimit(ref pa, precision);
