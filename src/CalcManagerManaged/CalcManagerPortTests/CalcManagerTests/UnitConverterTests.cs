...
public class UnitConverterTests : IDisposable
{
    // Helper methods to simplify unit test setup
    ...
    // Test implementation of IConverterDataLoader
    private class TestUnitConverterConfigLoader : IConverterDataLoader
    {
    ...
    // Test implementation of IUnitConverterVMCallback
    private class TestUnitConverterVMCallback : IUnitConverterVMCallback
    {
        private string _lastFrom = "";
        internal string _lastTo = "";
        private List<(string, Unit)> _lastSuggested = new();
        ...
        public void SuggestedValueCallback(List<(string, Unit)> suggestedValues)
        {
    ...
    // Test fixture for shared state
    private class UnitConverterTestFixture
    {
    ...
    public void UnitConverterTestBasic()
    {
        ...
        _fixture.UnitConverter.SendCommand(Command.Decimal);
        _fixture.TestVMCallback.CheckDisplayValues("30.", "30");
        Assert.True(_fixture.TestVMCallback.CheckSuggestedValues(test2));
    ...
    public void TestExtensiveConversionScenarios()
    {
        ...
        double finalValue = double.Parse(lbResult);
